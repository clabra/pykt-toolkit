...
class SimilarityAugmentedMultiHeadAttention(nn.Module):
    ...
    def forward(self, context_sequence, value_sequence, mask=None):
        ...
        # Reshape Q and K to (batch_size, seq_len, d_model)
        Q_sim = Q.transpose(1, 2).contiguous().view(batch_size, -1, self.d_model)
        K_sim = K.transpose(1, 2).contiguous().view(batch_size, -1, self.d_model)

        # Calculate similarity matrix (batch_size, batch_size)
        similarity = torch.matmul(Q_sim, K_sim.transpose(-2, -1))

        # Expand similarity to match attention scores shape
        similarity = similarity.unsqueeze(1).unsqueeze(2)

        # Add similarity to attention scores
