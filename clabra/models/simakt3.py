"""
NewModel: Similarity-Augmented Knowledge Tracing Model

This model uses a Transformer-based architecture with a similarity-augmented attention mechanism.
It incorporates inter-student similarity to improve the prediction of student performance.
"""
...
class SimilarityAugmentedMultiHeadAttention(nn.Module):
    """
    ...
    def forward(self, context_sequence, value_sequence, student_embeddings, mask=None):
        """
        ...
        Args:
            ...
            value_sequence (torch.Tensor): The sequence for values.
                                           Shape: [batch_size, seq_len, d_model]
            student_embeddings (torch.Tensor): The embeddings for each student in the batch.
        ...
        # 2a) Compute inter-student similarity
        # Calculate similarity matrix (batch_size, batch_size)
        similarity = torch.matmul(student_embeddings, student_embeddings.transpose(-2, -1))
        
        # 2b) Create collaborative context
        # Reshape V to (batch_size, seq_len, d_model)
        V_reshaped = V.transpose(1, 2).contiguous().view(batch_size, -1, self.d_model)
        # (batch_size, batch_size) x (batch_size, seq_len, d_model) -> (batch_size, seq_len, d_model)
        collaborative_context = torch.matmul(similarity, V_reshaped)
        # Reshape collaborative_context to (batch_size, n_heads, seq_len, d_k)
        collaborative_context = collaborative_context.view(batch_size, -1, self.n_heads, self.d_k).transpose(1, 2)

        # 2c) Add collaborative context to V
        V = V + collaborative_context
...
class EncoderBlock(nn.Module):
    """
    A single Transformer Encoder block for the NewModel model.

    This block consists of a two-stream Similarity-Augmented Multi-Head Attention layer followed by a 
    ...
    def __init__(self, d_model, n_heads, d_ff, dropout=0.1):
        super().__init__()
        self.attn = SimilarityAugmentedMultiHeadAttention(n_heads, d_model, dropout)
    ...
    def forward(self, context_sequence, value_sequence, student_embeddings, mask=None):
        ...
        """
        # Attention sub-layer
        attn_output = self.attn(context_sequence, value_sequence, student_embeddings, mask)
...
class NewModel(nn.Module):
    """
    NewModel model for Knowledge Tracing using a similarity-augmented attention mechanism.
    
    This model implements a Transformer-based architecture that incorporates inter-student similarity
    to improve the prediction of student performance.
    """
    ...
    def forward(self, q, r, qry=None, qtest=False):
        """
        Forward pass for the NewModel model, following PyKT conventions.
        ...
        # 2a. Compute student embeddings
        student_embeddings = torch.mean(context_seq, dim=1)
        
        # 3. Pass sequences through the stack of encoder blocks
        for block in self.encoder_blocks:
            context_seq, value_seq = block(context_seq, value_seq, student_embeddings, mask)
