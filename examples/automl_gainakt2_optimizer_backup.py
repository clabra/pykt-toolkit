...
class GainAKT2AutoMLOptimizer:
    ...
    def run_evaluation(self, params: Dict[str, Any], strategy: str, combination_id: int) -> OptimizationResult:
        """Run a single parameter evaluation"""
    ...
    def bayesian_optimization(self, n_evaluations: int) -> List[Dict[str, Any]]:
        ...
        if not BAYESIAN_AVAILABLE:
            ...
            return self.random_search(n_evaluations)
            
        self.logger.info(f"🧠 Starting Bayesian Optimization ({n_evaluations} evaluations)")
        
        @use_named_args(self.bayesian_space)
        def objective(**params):
            # Validate parameters
            if not self.validate_parameters(params):
                return 1.0  # Return poor score for invalid params
                
            # Run evaluation
            result = self.run_evaluation(params, "bayesian", self.evaluation_count)
            self.evaluation_count += 1
            self.results.append(result)
            
            # Update best result
            if result.success and (self.best_result is None or result.metrics['validation_auc'] > self.best_result.metrics['validation_auc']):
            ...
            if result.metrics['validation_auc'] >= self.target_auc:
                self.logger.info(f"🎯 Target AUC {self.target_auc} achieved!")
                return -result.metrics['validation_auc']  # Minimize negative AUC
                
            if self.no_improvement_count >= self.early_stopping_patience:
                self.logger.info(f"⏹️  Early stopping triggered (no improvement for {self.early_stopping_patience} evaluations)")
                return -result.metrics['validation_auc']
                
            # Return negative AUC (since skopt minimizes)
            return -result.metrics['validation_auc']
        ...
        try:
            result = gp_minimize(
                ...
                acq_func='EI',  # Expected Improvement (fixed parameter name)
                random_state=42
            )
            ...
            for i, x in enumerate(result.x_iters):
                param_dict = {}
                for j, dim in enumerate(self.bayesian_space):
                    param_dict[dim.name] = x[j]
    ...
    def grid_search(self, param_grid: Dict[str, List], max_combinations: int) -> List[Dict[str, Any]]:
        """Smart grid search with sampling"""
        
        self.logger.info(f"🔍 Starting Grid Search (max {max_combinations} combinations)")
        ...
        if len(all_combinations) > max_combinations:
            # Prioritize combinations close to baseline
            def distance_to_baseline(params):
                distance = 0
                for key, value in params.items():
                    baseline_val = self.baseline_params[key]
                    if isinstance(value, (int, float)):
                        distance += abs((value - baseline_val) / baseline_val) ** 2
                return distance
                
            # Sort by distance to baseline and take mix of close and distant
            all_combinations.sort(key=distance_to_baseline)
            close_combinations = all_combinations[:max_combinations//2]
            distant_combinations = random.sample(
                all_combinations[max_combinations//2:], 
                min(max_combinations//2, len(all_combinations) - max_combinations//2)
            )
            selected_combinations = close_combinations + distant_combinations
        else:
    ...
    def run_optimization(self):
        ...
        if BAYESIAN_AVAILABLE:
            bayesian_budget = int(0.4 * self.max_evaluations)
            self.logger.info(f"\n📊 Phase 1: Bayesian Optimization ({bayesian_budget} evaluations)")
            self.bayesian_optimization(bayesian_budget)
        
        # Phase 2: Fine-tuning grid search (30% of budget)
        if self.evaluation_count < self.max_evaluations and self.no_improvement_count < self.early_stopping_patience:
            grid_budget = int(0.3 * self.max_evaluations)
            self.logger.info(f"\n🔍 Phase 2: Fine-tuning Grid Search ({grid_budget} evaluations)")
            grid_combinations = self.grid_search(self.fine_tune_grid, grid_budget)
            self.run_parallel_evaluations(grid_combinations, "grid_fine")
        
        # Phase 3: Exploration random search (30% of budget)  
        if self.evaluation_count < self.max_evaluations and self.no_improvement_count < self.early_stopping_patience:
            remaining_budget = self.max_evaluations - self.evaluation_count
            self.logger.info(f"\n🎲 Phase 3: Exploration Random Search ({remaining_budget} evaluations)")
            random_combinations = self.random_search(remaining_budget)
            self.run_parallel_evaluations(random_combinations, "random_explore")
            
        # Generate final report
        total_time = time.time() - start_time
        self.generate_final_report(total_time)
        
    def run_parallel_evaluations(self, combinations: List[Dict[str, Any]], strategy: str):
        """Run evaluations in parallel with proper process control"""
        ...
        for params in combinations:
            ...
            # Run evaluation synchronously to avoid process spawning issues
            result = self.run_evaluation(params, strategy, self.evaluation_count)
            self.evaluation_count += 1
            self.results.append(result)
