"""
Generate Personalization Mosaic: 2x2 grid showing how Our Model personalizes predictions
for different students with identical response sequences, while BKT remains the same.

Each subplot shows:
- Student A (orange): Low P(L0), Low P(T) 
- Student B (blue): High P(L0), High P(T)
- BKT (gray): Same for both students (non-personalized)
- Ground truth: Green/red bars

The four plots demonstrate personalization across different response patterns.
"""
            ...
            for student_b in students[i+1:]:
                ...
                if student_a['p_l0'] < 0.65 and student_a['p_t'] < 0.1:
                    if student_b['p_l0'] > 0.7 and student_b['p_t'] > 0.12:
                        ...
                        contrast = (student_b['p_l0'] - student_a['p_l0']) + (student_b['p_t'] - student_a['p_t'])
                        divergence = np.mean(np.abs(student_a['preds'] - student_b['preds']))
                        score = contrast * 2 + divergence * 3
...
def plot_personalization_mosaic(pairs, bkt_params, output_path):
    ...
    plt.suptitle("Context-Aware Personalized Predictions vs. Non-Personalized BKT\nIdentical Sequences, Different Predictions", 
                 fontsize=16, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0.02, 0.02, 0.98, 0.95])
    plt.savefig(output_path, dpi=300)
    print(f"Saved personalization mosaic to {output_path}")
...
def main():
    ...
    # Load configuration - handle both direct config.json and nested structure
    config_path = os.path.join(args.exp_dir, 'config.json')
    if not os.path.exists(config_path):
        # Try to find config.json in nested structure (e.g., gtransformer/assist2009/fold_0_xxx/config.json)
        import glob
        config_candidates = glob.glob(os.path.join(args.exp_dir, '**/config.json'), recursive=True)
        if config_candidates:
            config_path = config_candidates[0]  # Use first found config
        else:
            raise FileNotFoundError(f"config.json not found in {args.exp_dir} or subdirectories")
    
    with open(config_path, 'r') as f:
        config = json.load(f)
    
    # Extract model config and dataset name - handle both old and new config structures
    if 'train_config' in config:
        # New structure: use train_config which contains merged defaults + input
        model_config = config['train_config']
        dataset_name = config.get('input', {}).get('dataset', config['train_config'].get('dataset_name', 'assist2009'))
    elif 'model_config' in config:
        # Old structure
        model_config = config['model_config']
        dataset_name = config.get('dataset_name', 'assist2009')
    else:
        # Fallback: try to merge defaults and input
        model_config = {**config.get('defaults', {}), **config.get('input', {})}
        dataset_name = config.get('input', {}).get('dataset', config.get('params', {}).get('dataset_name', 'assist2009'))
    ...
    # Load BKT parameters - check dpath and parent directory (for nips_task34)
    bkt_path = os.path.join(dpath, "bkt_skill_params.pkl")
    if not os.path.exists(bkt_path):
        bkt_path = os.path.join(os.path.dirname(dpath), "bkt_skill_params.pkl")
    ...
    import glob
    checkpoint_candidates = glob.glob(os.path.join(args.exp_dir, '**/qid_model.ckpt'), recursive=True)
    ...
    output_path = os.path.join(args.output_dir, 'personalization_mosaic.png')
