...
def parse_args():
    ...
    parser.add_argument("--bkt_preds", type=str, required=False, help="Path to BKT predictions CSV (traj_predictions.csv). If not provided, will auto-discover.")
...
def main():
    ...
    # Auto-discover BKT predictions file if not provided
    if args.bkt_preds is None:
        # Try to find BKT predictions in the data directory
        dataset_dpath = data_config[args.dataset]['dpath']
        bkt_preds_candidates = [
            os.path.join(dataset_dpath, 'traj_predictions_bkt.csv'),
            os.path.join(dataset_dpath, 'traj_predictions.csv'),
            os.path.join(dataset_dpath, f'bkt_predictions_fold{args.fold}.csv'),
        ]
        for candidate in bkt_preds_candidates:
            if os.path.exists(candidate):
                args.bkt_preds = candidate
                print(f"Auto-discovered BKT predictions: {candidate}")
                break
        
        if args.bkt_preds is None:
            raise FileNotFoundError(
                f"BKT predictions file not found!\n"
                f"Searched in: {dataset_dpath}\n"
                f"Expected filenames: traj_predictions_bkt.csv, traj_predictions.csv\n"
                f"\nThis script requires BKT trajectory predictions generated during model evaluation.\n"
                f"Please provide --bkt_preds argument pointing to a valid BKT predictions file."
            )
