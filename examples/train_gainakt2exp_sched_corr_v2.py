#!/usr/bin/env python3
"""Scheduled GainAKT2Exp Training v2

Enhancements over v1 (`train_gainakt2exp_sched_corr.py`):
1. Variance instrumentation: logs per-epoch mean/std of per-student mastery sequence variance.
2. Dynamic correlation target schedule: target ladder (t0 -> t1 -> t2) triggered by sustained correlation achievements.
3. Constraint staging: only monotonicity + performance losses active until scale_threshold; sparsity & consistency deferred.
4. Plateau / regression detection: optional early stop if mastery corr drops > reg_drop_pct from rolling max for patience epochs.

Rationale: Preserve variance during emergence, avoid premature compression, adaptively increase ambition, and abort regressive runs early.

Usage example:
  python examples/train_gainakt2exp_sched_corr_v2.py \\
    --dataset assist2015 --epochs 15 --batch_size 96 --lr 1.74e-4 \\
    --enable_constraints --constraint_warmup_epochs 3 --constraint_ramp_epochs 8 \\
    --staging_scale_threshold 0.5 --corr_regularizer_weight 0.3 \\
    --corr_targets 0.08 0.09 0.10 --corr_promote_epochs 2 \\
    --corr_start_epoch 4 --union_sampling --global_students 600 \\
    --early_stop_patience 3 --regression_drop_pct 0.15 --seed 21 \\
    --output_suffix v2_seed21

Outputs:
  - Config JSON & semantic trajectory JSON in paper/results/sched_corr_runs_v2/
  - Each trajectory entry includes variance stats & active target.
...
def compute_global_correlations(records, strat_target, seed, union_sampling=False):
    ...
    return (float(np.mean(final_m)) if final_m else 0.0,
            ...
            {
                'acc_mastery_count': len(acc_m), 'acc_gain_count': len(acc_g),
                'strat_mastery_count': len(strat_m), 'strat_gain_count': len(strat_g),
                'final_mastery_count': len(final_m), 'final_gain_count': len(final_g)
            })
...
# ---------------- Main -----------------
def main():
    ap = argparse.ArgumentParser(description="GainAKT2Exp Scheduled v2")
    ...
    ap.add_argument('--staging_scale_threshold', type=float, default=0.5, help='Activate sparsity/consistency only after this scale.')
    ap.add_argument('--union_sampling', action='store_true')
    ap.add_argument('--global_students', type=int, default=600)
    ap.add_argument('--corr_regularizer_weight', type=float, default=0.3)
    ap.add_argument('--corr_start_epoch', type=int, default=4)
    ap.add_argument('--corr_targets', type=float, nargs='+', default=[0.08,0.09,0.10])
    ap.add_argument('--corr_promote_epochs', type=int, default=2, help='Consecutive epochs meeting target before promotion.')
    ap.add_argument('--device', default='auto')
    ap.add_argument('--early_stop_patience', type=int, default=3)
    ap.add_argument('--regression_drop_pct', type=float, default=0.15)
    ...
    data_config = {
        'assist2015': {
            'dpath':'/workspaces/pykt-toolkit/data/assist2015','num_q':0,'num_c':100,'input_type':['concepts'],'max_concepts':1,'min_seq_len':3,'maxlen':200,'emb_path':'',
            'train_valid_original_file':'train_valid.csv','train_valid_file':'train_valid_sequences.csv','folds':[0,1,2,3,4],
            'test_original_file':'test.csv','test_file':'test_sequences.csv','test_window_file':'test_window_sequences.csv'
        }
    }
    ...
    run_dir = 'paper/results/sched_corr_runs_v2'
    os.makedirs(run_dir, exist_ok=True)
    run_id = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
    suffix = args.output_suffix or f'v2_seed{args.seed}'
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        preds_all = []
        targets_all = []
        batch_mastery_variances = []  # per-student variance aggregated across sequence positions
            ...
            if args.enable_constraints and constraint_scale > 0 and 'interpretability_loss' in out:
                ...
                if breakdown:
                    for k, v in breakdown.items():
                        if ('sparsity' in k or 'consistency' in k) and constraint_scale < args.staging_scale_threshold:
                            continue
                        total_loss += v * constraint_scale
                else:
                    total_loss += out['interpretability_loss'] * constraint_scale
            if epoch >= args.corr_start_epoch and args.corr_regularizer_weight > 0 and 'projected_mastery' in out:
                pm = out['projected_mastery']
                perf = rsh
                batch_records = []
                ...
                if reg is not None:
                    total_loss += reg * args.corr_regularizer_weight
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()
        ...
        if gm > mastery_corr_max:
            mastery_corr_max = gm
            regression_patience = 0
        else:
            if mastery_corr_max > 0 and (mastery_corr_max - gm)/mastery_corr_max >= args.regression_drop_pct:
        ...
        print(f"[Epoch {epoch:02d}] TL={train_loss:.4f} TAUC={train_auc:.4f} VAUC={val_auc:.4f} MCorr={gm:.4f} GCorr={gg:.4f} VarMean={mastery_var_mean:.5f} Target={current_target:.3f} Scale={constraint_scale:.2f} RegPat={regression_patience}")

        if args.early_stop_patience > 0 and regression_patience >= args.early_stop_patience:
            print(f"[EarlyStop] Mastery corr regressed >= {args.regression_drop_pct*100:.1f}% for {regression_patience} epochs. Stopping early.")
            # Append an early_stop flag in last record
            semantic_traj[-1]['early_stop'] = True
            break
