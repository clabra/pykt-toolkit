#!/usr/bin/env python3
"""Scheduled GainAKT2Exp Training v3

Additions over v2:
1. Delayed performance alignment: mastery/gain performance losses inactive until performance_delay_scale.
2. Variance floor penalty: encourages maintaining minimal mastery variance (VarMean >= variance_floor).
3. Separate sparsity ramp: sparsity loss scaled with sparsity_subscale independent of main constraint_scale.
4. Adaptive correlation weight boost: if MCorr declines for decay_patience epochs while scale increases, temporarily raise corr weight.
5. Relaxed target ladder advancement: promote when MCorr >= target - target_margin for promote_epochs.
6. Logging of additional subscales and adaptive_corr_weight.

Objective: Sustain mastery correlation > 0.06 beyond mid-ramp while preventing extreme variance collapse and limiting AUC degradation.

Usage example:
  python examples/train_gainakt2exp_sched_corr_v3.py \
    ...
    --variance_floor 0.001 --variance_floor_weight 0.05 \
    --sparsity_ramp_start 0.4 --sparsity_ramp_end 0.8 \
    --corr_regularizer_weight 0.3 --corr_weight_boost 0.1 --decay_patience 2 \
    ...
    --seed 21 --output_suffix v3_seed21
...
def main():
    ap = argparse.ArgumentParser(description="GainAKT2Exp Scheduled v3")
    # Core args
    ...
    ap.add_argument('--performance_delay_scale', type=float, default=0.35, help='Delay performance alignment until constraint_scale >= this.')
    # Variance floor
    ap.add_argument('--variance_floor', type=float, default=0.001)
    ap.add_argument('--variance_floor_weight', type=float, default=0.05)
    # Sparsity ramp
    ap.add_argument('--sparsity_ramp_start', type=float, default=0.4)
    ap.add_argument('--sparsity_ramp_end', type=float, default=0.8)
    # Correlation regularization
    ap.add_argument('--corr_regularizer_weight', type=float, default=0.3)
    ap.add_argument('--corr_weight_boost', type=float, default=0.1)
    ap.add_argument('--decay_patience', type=int, default=2, help='Consecutive decline epochs to trigger weight boost.')
    ap.add_argument('--corr_start_epoch', type=int, default=4)
    ap.add_argument('--corr_targets', type=float, nargs='+', default=[0.08,0.09,0.10])
    ap.add_argument('--target_margin', type=float, default=0.01, help='Margin below target to count towards promotion.')
    ...
    run_dir = 'paper/results/sched_corr_runs_v3'
    os.makedirs(run_dir, exist_ok=True)
    run_id = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
    suffix = args.output_suffix or f'v3_seed{args.seed}'
    ...
    for epoch in range(1, args.epochs+1):
        ...
        adaptive_corr_weight = args.corr_regularizer_weight
        prev_mcorr = semantic_traj[-1]['global_alignment_mastery_corr'] if semantic_traj else None
        ...
        for batch in train_loader:
            ...
            if args.enable_constraints and constraint_scale > 0 and 'interpretability_loss_breakdown' in out:
                for k,v in out['interpretability_loss_breakdown'].items():
                    # Delay performance alignment until scale threshold
            ...
            if epoch >= args.corr_start_epoch and adaptive_corr_weight > 0 and 'projected_mastery' in out:
                pm = out['projected_mastery']
                perf = rsh
                batch_records = []
                ...
                if reg is not None:
                    total_loss += reg * adaptive_corr_weight
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()
        ...
        # Adaptive corr weight boost (post-hoc; we log intention—applies next epoch)
        if prev_mcorr is not None and gm < prev_mcorr and constraint_scale > 0:
            decay_counter += 1
        else:
            decay_counter = 0

        boost_applied = False
        if decay_counter >= args.decay_patience:
            adaptive_corr_weight += args.corr_weight_boost
            boost_applied = True
            decay_counter = 0  # reset after boost trigger

        # Relaxed target promotion
        ...
        # Variance floor penalty (tracked only)
        variance_floor_penalty = max(0.0, args.variance_floor - mastery_var_mean) * args.variance_floor_weight if epoch >= args.corr_start_epoch else 0.0
        ...
        semantic_traj.append({
            ...
            'adaptive_corr_weight': adaptive_corr_weight,
            'boost_applied': boost_applied,
            'variance_floor_penalty': variance_floor_penalty,
            'counts': counts
        })
        print(f"[Epoch {epoch:02d}] TL={train_loss:.4f} TAUC={train_auc:.4f} VAUC={val_auc:.4f} MCorr={gm:.4f} GCorr={gg:.4f} VarMean={mastery_var_mean:.5f} Target={current_target:.3f} Scale={constraint_scale:.2f} CorrW={adaptive_corr_weight:.2f} Boost={int(boost_applied)} VarPen={variance_floor_penalty:.5f}")
