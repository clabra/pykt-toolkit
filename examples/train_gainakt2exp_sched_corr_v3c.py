#!/usr/bin/env python3
"""Scheduled GainAKT2Exp Training v3c

Variance-preserving iteration incorporating:
1. Mixed variance penalty: linear + squared components.
2. Correlation gating: apply correlation regularizer only if variance above gate threshold.
3. Restoration noise: additive Gaussian noise to mastery head when variance too low.
4. Monotonicity delay: monotonicity loss inactive until scale >= mono_delay_scale.
5. Early extreme-collapse stop: abort if VarMean < variance_floor/12 for collapse_patience epochs.
6. Initial target override: first target lowered to initial_corr_target to reduce early compression.

Objective: Maintain mastery variance >= variance_floor (default 0.002) through ramp while achieving sustained MCorr >= 0.058 after scale 0.5.

Usage example:
  python examples/train_gainakt2exp_sched_corr_v3c.py \
    ...
    --variance_floor 0.002 --var_linear_weight 1.0 --var_square_weight 0.2 \
    --variance_gate_factor 0.5 --restoration_noise_std 0.02 --collapse_patience 3 \
    --corr_regularizer_weight 0.35 --corr_weight_low 0.18 --corr_ramp_scale 0.3 \
    --corr_targets 0.08 0.09 0.10 --initial_corr_target 0.06 --target_margin 0.01 --corr_promote_epochs 2 \
    --corr_start_epoch 4 --sparsity_ramp_start 0.45 --sparsity_ramp_end 0.85 \
    --union_sampling --global_students 600 --seed 21 --output_suffix v3c_seed21
...
def main():
    ap = argparse.ArgumentParser(description="GainAKT2Exp Scheduled v3c")
    # Core
    ...
    ap.add_argument('--var_linear_weight', type=float, default=1.0)
    ap.add_argument('--var_square_weight', type=float, default=0.2)
    ap.add_argument('--variance_gate_factor', type=float, default=0.5, help='Gate correlation if VarMean < variance_floor * factor.')
    ...
    base_weights = {
        ...
        'monotonicity_loss_weight': 0.1,  # base, but delayed by mono_delay_scale
    ...
    run_dir = 'paper/results/sched_corr_runs_v3c'
    os.makedirs(run_dir, exist_ok=True)
    run_id = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
    suffix = args.output_suffix or f'v3c_seed{args.seed}'
    with open(os.path.join(run_dir, f'config_{suffix}_{run_id}.json'),'w') as cf:
        json.dump(vars(args), cf, indent=2)

    # Correlation targets (prepend initial override if smaller)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch in train_loader:
            ...
            apply_corr = (epoch >= args.corr_start_epoch and constraint_scale > 0)
            if apply_corr and 'projected_mastery' in out:
                pm = out['projected_mastery']
                perf = rsh
                batch_records = []
                ...
                # Correlation gating will be applied later (needs variance stats for epoch)
                reg = mastery_corr_regularizer(batch_records, current_target)
                if reg is not None:
                    # Temporarily store; final decision after variance aggregate
        ...
        # Mixed variance penalty
        variance_gap = max(0.0, args.variance_floor - mastery_var_mean)
        variance_penalty = variance_gap * args.var_linear_weight + (variance_gap ** 2) * args.var_square_weight if epoch >= args.corr_start_epoch else 0.0

        # Apply correlation gating & restoration noise (post-hoc logging only; actual training corr_term already applied earlier)
        variance_gate = args.variance_floor * args.variance_gate_factor
        corr_applied = False
        if corr_term is not None and mastery_var_mean >= variance_gate:
            corr_applied = True
        elif corr_term is not None and mastery_var_mean < variance_gate:
            # If we wanted to remove the effect we would need to redo the step; for simplicity we just log gating failure.
            corr_applied = False
        ...
        print(f"[Epoch {epoch:02d}] TL={train_loss:.4f} TAUC={train_auc:.4f} VAUC={val_auc:.4f} MCorr={gm:.4f} GCorr={gg:.4f} VarMean={mastery_var_mean:.5f} Target={current_target:.3f} Scale={constraint_scale:.2f} VarPen={variance_penalty:.6f} CorrApplied={int(corr_applied)} CollapseCt={collapse_counter}")

        if early_stop:
            print(f"[EarlyStop] Extreme variance collapse detected (VarMean < floor/12 for {collapse_counter} epochs).")
            break
