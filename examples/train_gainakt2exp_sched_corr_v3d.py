#!/usr/bin/env python3
"""Scheduled GainAKT2Exp Training v3d

Enhancements over v3c:
1. Strong variance penalty: linear + squared with higher default weights.
2. Recovery freeze: when variance below gate for 2 consecutive epochs, freeze monotonicity & performance alignment losses until recovered.
3. Promotion guard: correlation target promotion only if VarMean >= variance_floor.
4. Restoration noise applied directly to projected mastery when VarMean < gate.
5. Interpretability loss breakdown logging (per component magnitudes) for diagnostics.
6. Correlation regularizer skipped entirely while in recovery freeze.

Objective: Maintain VarMean >= variance_floor (default 0.002) through ramp while achieving sustained MCorr ≥ 0.058 post scale 0.5.

Usage example:
  python examples/train_gainakt2exp_sched_corr_v3d.py \
    ...
    --variance_gate_factor 0.5 --restoration_noise_std 0.02 --recovery_patience 2 \
    --collapse_patience 3 --corr_regularizer_weight 0.35 --corr_weight_low 0.15 \
    --corr_ramp_scale 0.3 --initial_corr_target 0.06 --corr_targets 0.08 0.09 0.10 \
    --target_margin 0.01 --corr_promote_epochs 2 --corr_start_epoch 4 \
    --sparsity_ramp_start 0.45 --sparsity_ramp_end 0.85 --union_sampling --global_students 600 \
    --seed 21 --output_suffix v3d_seed21
...
def main():
    ap = argparse.ArgumentParser(description="GainAKT2Exp Scheduled v3d")
    ...
    ap.add_argument('--corr_weight_low', type=float, default=0.15)
    ...
    run_dir = 'paper/results/sched_corr_runs_v3d'
    os.makedirs(run_dir, exist_ok=True)
    run_id = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
    suffix = args.output_suffix or f'v3d_seed{args.seed}'
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch in train_loader:
            ...
            mask_bool = msk.bool()
            if pm is not None and recovery_freeze and pm.requires_grad:
                with torch.no_grad():
                    noise = torch.randn_like(pm) * args.restoration_noise_std
                    pm.add_(noise)
            ...
            if args.enable_constraints and constraint_scale > 0 and 'interpretability_loss_breakdown' in out:
                for k,v in out['interpretability_loss_breakdown'].items():
                    ...
                    if ('performance' in k) and (constraint_scale < args.performance_delay_scale or recovery_freeze):
                        interpretability_components_logged.append((k,comp_val,0.0))
                        continue
                    if 'monotonicity' in k and (constraint_scale < args.mono_delay_scale or recovery_freeze):
                        interpretability_components_logged.append((k,comp_val,0.0))
                        continue
                    if 'sparsity' in k:
                        ...
                        continue
                    if 'consistency' in k and (constraint_scale < args.staging_scale_threshold or recovery_freeze):
                        interpretability_components_logged.append((k,comp_val,0.0))
                        continue
                    total_loss += v * constraint_scale
                    interpretability_components_logged.append((k,comp_val,constraint_scale))
            elif args.enable_constraints and constraint_scale > 0 and 'interpretability_loss' in out and not recovery_freeze:
                total_loss += out['interpretability_loss'] * constraint_scale

            apply_corr = (epoch >= args.corr_start_epoch and constraint_scale > 0 and not recovery_freeze)
            corr_term = None
        ...
        if epoch >= args.corr_start_epoch and gm >= (current_target - args.target_margin) and mastery_var_mean >= args.variance_floor:
            promote_counter += 1
        ...
        semantic_traj.append({
            ...
            'variance_penalty': epoch_variance_penalty_mean,'variance_gate': variance_gate,'recovery_freeze': recovery_freeze,
        ...
        print(f"[Epoch {epoch:02d}] TL={train_loss:.4f} TAUC={train_auc:.4f} VAUC={val_auc:.4f} MCorr={gm:.4f} VarMean={mastery_var_mean:.5f} Target={current_target:.3f} Scale={constraint_scale:.2f} VarPen={epoch_variance_penalty_mean:.5f} RecFreeze={int(recovery_freeze)} CollapseCt={collapse_counter}")
        if early_stop:
