#!/usr/bin/env python3
"""
Standardized training script for GainAKT3Exp model using PyKT framework patterns.
...
def validate_model_consistency(model, data_loader, device, logger, max_students=100):
    ...
    # COMMENTED OUT (2025-11-16): Correlation tracking removed
    # correlations = {
    #     'mastery_performance': [],
    #     'gain_performance': []
    # }
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            # COMMENTED OUT (2025-11-16): Not used since correlation code is commented out
            ...
            for i in range(batch_size_actual):
                ...
                # COMMENTED OUT (2025-11-16): Correlation calculation
                # Reason: Use loss components and encoder AUCs for correlation measures instead
                # See: tmp/CORRELATION_ANALYSIS_DUAL_ENCODER.md for detailed analysis
                # Correlations - only for practiced skills
                # if seq_len >= 3:
                #     try:
                ...
                #             practiced_skills.add(skill_id)
                #
                #         practiced_skills = list(practiced_skills)
                #         
                #         # DEBUG: Log mastery statistics for first few students
                #         if student_count < 3:  # Log first 3 students
                #             logger.info(f"[DEBUG] Student {student_count}: {len(practiced_skills)} practiced skills")
                #             logger.info(f"[DEBUG] Mastery range: {mastery_np.min():.3f} - {mastery_np.max():.3f}")
                #             logger.info(f"[DEBUG] Mastery std: {np.std(mastery_np):.3f}")
                #             logger.info(f"[DEBUG] Performance: {performance_np}")
                #             if len(practiced_skills) > 0:
                #                 practiced_mastery = mastery_np[:, practiced_skills]
                #                 logger.info(f"[DEBUG] Practiced mastery range: {practiced_mastery.min():.3f} - {practiced_mastery.max():.3f}")
                #         
                #         if len(practiced_skills) > 0:
                #             # Average mastery only across practiced skills
                #             practiced_mastery = mastery_np[:, practiced_skills]  # [seq_len, num_practiced_skills]
                #             mean_mastery_practiced = np.mean(practiced_mastery, axis=1)  # [seq_len]
                #
                #             mastery_corr = np.corrcoef(mean_mastery_practiced, performance_np)[0, 1]
                #             if not np.isnan(mastery_corr):
                #                 correlations['mastery_performance'].append(mastery_corr)
                #
                #         # Alternative: Per-skill correlation (more granular)
                #         per_skill_correlations = []
                #         for skill_idx in practiced_skills:
                #             skill_mastery_progression = mastery_np[:, skill_idx]
                #             # Only compute if there's variance in mastery progression
                #             if np.std(skill_mastery_progression) > 1e-6:
                #                 skill_corr = np.corrcoef(skill_mastery_progression, performance_np)[0, 1]
                #                 if not np.isnan(skill_corr):
                #                     per_skill_correlations.append(skill_corr)
                #         
                #         if per_skill_correlations and student_count < 3:
                #             avg_per_skill_corr = np.mean(per_skill_correlations)
                #             logger.info(f"[DEBUG] Student {student_count}: Per-skill mastery correlation: {avg_per_skill_corr:.3f}")
                #
                #
                #         # For gains, use the same approach
    ...
    # COMMENTED OUT (2025-11-16): Correlation metrics removed
    ...
    # COMMENTED OUT (2025-11-16): Correlation logging removed
...
def train_gainakt3exp_model(args):
    """
    Standardized training function for GainAKT3Exp model using PyKT framework patterns.
    ...
    # =====================================================================
    # DEPRECATED PARAMETER LOADING (2025-11-16)
    # All semantic/constraint features disabled in dual-encoder architecture
    # These parameters are no longer in argparse or parameter_default.json
    # =====================================================================
    # # Alignment / semantic emergence arguments (DEPRECATED 2025-11-16: all disabled, no longer in argparse)
    # enable_alignment_loss = resolve_param(cfg, 'alignment', 'enable_alignment_loss', False)  # DEPRECATED: no argparse entry
    # alignment_weight = float(resolve_param(cfg, 'alignment', 'alignment_weight', 0.0))  # DEPRECATED: no argparse entry
    # alignment_warmup_epochs = int(resolve_param(cfg, 'alignment', 'alignment_warmup_epochs', 8))
    # adaptive_alignment = resolve_param(cfg, 'alignment', 'adaptive_alignment', False)  # DEPRECATED: no argparse entry
    # alignment_min_correlation = float(resolve_param(cfg, 'alignment', 'alignment_min_correlation', 0.05))
    # # Global alignment / residual options (DEPRECATED 2025-11-16: all disabled, no longer in argparse)
    # enable_global_alignment_pass = resolve_param(cfg, 'global_alignment', 'enable_global_alignment_pass', False)  # DEPRECATED: no argparse entry
    # alignment_global_students = int(resolve_param(cfg, 'global_alignment', 'alignment_global_students', 600))
    # use_residual_alignment = resolve_param(cfg, 'global_alignment', 'use_residual_alignment', False)  # DEPRECATED: no argparse entry
    # alignment_residual_window = int(resolve_param(cfg, 'global_alignment', 'alignment_residual_window', 5))
    # # Refinement cycle arguments (DEPRECATED 2025-11-16: all disabled, no longer in argparse)
    # enable_retention_loss = resolve_param(cfg, 'refinement', 'enable_retention_loss', False)  # DEPRECATED: no argparse entry
    # retention_delta = float(resolve_param(cfg, 'refinement', 'retention_delta', 0.005))
    # retention_weight = float(resolve_param(cfg, 'refinement', 'retention_weight', 0.0))  # DEPRECATED: no argparse entry
    # enable_lag_gain_loss = resolve_param(cfg, 'refinement', 'enable_lag_gain_loss', False)  # DEPRECATED: no argparse entry
    # lag_gain_weight = float(resolve_param(cfg, 'refinement', 'lag_gain_weight', 0.0))  # DEPRECATED: no argparse entry
    # lag_max_lag = int(resolve_param(cfg, 'refinement', 'lag_max_lag', 3))
    # # Weighted multi-lag scheme (L1 emphasis)
    # lag_l1_weight = float(resolve_param(cfg, 'refinement', 'lag_l1_weight', 0.5))
    # lag_l2_weight = float(resolve_param(cfg, 'refinement', 'lag_l2_weight', 0.3))
    # lag_l3_weight = float(resolve_param(cfg, 'refinement', 'lag_l3_weight', 0.2))
    # # Alignment share cap & decay factor
    # alignment_share_cap = float(resolve_param(cfg, 'alignment', 'alignment_share_cap', 0.08))
    # alignment_share_decay_factor = float(resolve_param(cfg, 'alignment', 'alignment_share_decay_factor', 0.7))
    # enable_cosine_perf_schedule = resolve_param(cfg, 'runtime', 'enable_cosine_perf_schedule', False)  # DEPRECATED: no argparse entry
    # consistency_rebalance_epoch = int(resolve_param(cfg, 'refinement', 'consistency_rebalance_epoch', 8))
    # consistency_rebalance_threshold = float(resolve_param(cfg, 'refinement', 'consistency_rebalance_threshold', 0.10))
    # consistency_rebalance_new_weight = float(resolve_param(cfg, 'refinement', 'consistency_rebalance_new_weight', 0.2))
    # variance_floor = float(resolve_param(cfg, 'refinement', 'variance_floor', 1e-4))
    # variance_floor_patience = int(resolve_param(cfg, 'refinement', 'variance_floor_patience', 3))
    # variance_floor_reduce_factor = float(resolve_param(cfg, 'refinement', 'variance_floor_reduce_factor', 0.5))
    # # Individual constraint weights (DEPRECATED 2025-11-16: all disabled, no longer in argparse)
    # non_negative_loss_weight = resolve_param(cfg, 'interpretability', 'non_negative_loss_weight', 0.0)  # DEPRECATED: no argparse entry
    # monotonicity_loss_weight = resolve_param(cfg, 'interpretability', 'monotonicity_loss_weight', 0.0)  # DEPRECATED: no argparse entry
    # mastery_performance_loss_weight = resolve_param(cfg, 'interpretability', 'mastery_performance_loss_weight', 0.0)  # DEPRECATED: no argparse entry
    # gain_performance_loss_weight = resolve_param(cfg, 'interpretability', 'gain_performance_loss_weight', 0.0)  # DEPRECATED: no argparse entry
    # sparsity_loss_weight = resolve_param(cfg, 'interpretability', 'sparsity_loss_weight', 0.0)  # DEPRECATED: no argparse entry
    # consistency_loss_weight = resolve_param(cfg, 'interpretability', 'consistency_loss_weight', 0.0)  # DEPRECATED: no argparse entry
    # =====================================================================
    ...
    logger.info("GainAKT3Exp dual-encoder: BCE + Incremental Mastery Loss only")
    logger.info(f"  BCE loss weight (λ₁): {bce_loss_weight}")
    logger.info(f"  Incremental Mastery loss weight (λ₂): {incremental_mastery_loss_weight}")
    # =====================================================================
    # DEPRECATED LOGGING (2025-11-16) - Commented out with deprecated features
    # =====================================================================
    ...
    if use_wandb:
        ...
        wandb.init(project="pykt-cumulative-mastery", name=f"gainakt3exp_{experiment_suffix}", mode="offline")
    ...
    model_config = {
        ...
        # DEPRECATED (2025-11-16): intrinsic_gain_attention removed from GainAKT3Exp
            ...
            if not os.path.exists(repro_metrics_csv):
                with open(repro_metrics_csv, 'w', newline='') as fcsv:
                    ...
                    writer.writerow([
                        ...
                        'train_weighted_bce','train_weighted_im','train_total_weighted',
                        'val_weighted_bce','val_weighted_im','val_total_weighted',
                        # Loss shares (percentage contribution)
                        'train_bce_share','train_im_share',
                        'val_bce_share','val_im_share',
                        ...
                        # REMOVED (2025-11-16): mastery_correlation, gain_correlation
                        # Reason: Encoder AUC metrics and loss components provide better correlation measures
                        # mastery_correlation computed incorrect average across all skills (not skill-specific)
                        # gain_correlation not applicable (use_gain_head=false, gains not exposed)
                        # See: tmp/CORRELATION_ANALYSIS_DUAL_ENCODER.md for detailed analysis
    ...
    for epoch in range(num_epochs):
        ...
        # DUAL-ENCODER METRICS (2025-11-16): Separate prediction tracking for both encoders
        total_predictions = []  # Combined (for legacy compatibility)
        total_targets = []
        total_predictions_encoder1 = []  # Encoder 1: Base predictions from performance path
        total_predictions_encoder2 = []  # Encoder 2: Incremental mastery predictions from interpretability path
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # DUAL-ENCODER METRICS (2025-11-16): Collect predictions from both encoders
            with torch.no_grad():
                # Encoder 1: Base predictions (from logits)
                probs_encoder1 = torch.sigmoid(valid_predictions)
                total_predictions_encoder1.extend(probs_encoder1.cpu().numpy())
                
                # Encoder 2: Incremental mastery predictions (if available)
                if 'incremental_mastery_predictions' in outputs:
                    # Extract incremental mastery predictions with same mask
                    im_logits = outputs['incremental_mastery_predictions']
                    if mask.sum() > 0:
                        valid_im_predictions = im_logits[valid_mask]
                        total_predictions_encoder2.extend(valid_im_predictions.cpu().numpy())
                    else:
                        # If no valid positions, add zeros to maintain alignment
                        total_predictions_encoder2.extend(np.zeros(len(valid_targets)))
                else:
                    # If incremental mastery predictions not available, use zeros
                    total_predictions_encoder2.extend(np.zeros(len(valid_targets.cpu().numpy())))
                
                # Legacy combined predictions (Encoder 1 for backward compatibility)
                total_predictions.extend(probs_encoder1.cpu().numpy())
        ...
        # Validation phase
        model.eval()
        # DUAL-ENCODER METRICS (2025-11-16): Separate validation tracking for both encoders
        val_predictions = []  # Combined (legacy)
        val_targets = []
        val_predictions_encoder1 = []  # Encoder 1: Base predictions
        val_predictions_encoder2 = []  # Encoder 2: Incremental mastery predictions
        val_loss = 0.0
        val_main_loss_total = 0.0
        val_incremental_mastery_loss_total = 0.0
        ...
        with torch.no_grad():
            for batch in valid_loader:
                ...
                val_loss += loss.item()
                val_main_loss_total += loss.item()
                
                # Track incremental mastery loss if available
                incremental_mastery_loss_val = outputs.get('incremental_mastery_loss', torch.tensor(0.0, device=device))
                val_incremental_mastery_loss_total += incremental_mastery_loss_val.item()
                
                # DUAL-ENCODER METRICS (2025-11-16): Collect predictions from both encoders
                # Encoder 1: Base predictions (from logits)
                probs_encoder1 = torch.sigmoid(valid_predictions)
                val_predictions_encoder1.extend(probs_encoder1.cpu().numpy())
                ...
                if 'incremental_mastery_predictions' in outputs:
                    ...
                    if mask.sum() > 0:
                        ...
                        val_predictions_encoder2.extend(valid_im_preds.cpu().numpy())
                    else:
                        val_predictions_encoder2.extend(np.zeros(len(valid_targets)))
                else:
                    val_predictions_encoder2.extend(np.zeros(len(valid_targets.cpu().numpy())))
                
                # Legacy combined predictions (Encoder 1)
                val_predictions.extend(probs_encoder1.cpu().numpy())
        ...
        # DUAL-ENCODER METRICS (2025-11-16): Compute encoder-specific performance
        # Training metrics for both encoders
        train_stats_encoder1 = compute_auc_acc(total_targets, total_predictions_encoder1)
        train_auc_encoder1 = train_stats_encoder1['auc']
        train_acc_encoder1 = train_stats_encoder1['acc']
        
        if len(total_predictions_encoder2) > 0 and len(total_predictions_encoder2) == len(total_targets):
            train_stats_encoder2 = compute_auc_acc(total_targets, total_predictions_encoder2)
            train_auc_encoder2 = train_stats_encoder2['auc']
            train_acc_encoder2 = train_stats_encoder2['acc']
        else:
            train_auc_encoder2 = 0.0
            train_acc_encoder2 = 0.0
        
        # Validation metrics for both encoders
        val_stats_encoder1 = compute_auc_acc(val_targets, val_predictions_encoder1)
        val_auc_encoder1 = val_stats_encoder1['auc']
        val_acc_encoder1 = val_stats_encoder1['acc']
        
        if len(val_predictions_encoder2) > 0 and len(val_predictions_encoder2) == len(val_targets):
            val_stats_encoder2 = compute_auc_acc(val_targets, val_predictions_encoder2)
            val_auc_encoder2 = val_stats_encoder2['auc']
            val_acc_encoder2 = val_stats_encoder2['acc']
        else:
            val_auc_encoder2 = 0.0
            val_acc_encoder2 = 0.0
        
        # Compute average losses (unweighted)
        val_main_loss_avg = val_main_loss_total / len(valid_loader)
        val_incremental_mastery_loss_avg = val_incremental_mastery_loss_total / len(valid_loader)
        
        # Compute weighted losses for validation
        val_weighted_bce = bce_loss_weight * val_main_loss_avg
        val_weighted_im = incremental_mastery_loss_weight * val_incremental_mastery_loss_avg
        val_total_weighted = val_weighted_bce + val_weighted_im
        
        # Compute loss shares for validation
        if val_total_weighted > 0:
            val_bce_share = val_main_loss_avg / val_total_weighted
            val_im_share = val_incremental_mastery_loss_avg / val_total_weighted
        else:
            val_bce_share = val_im_share = 0.0
        
        # Compute weighted training losses
        train_weighted_bce = bce_loss_weight * train_main_loss
        train_weighted_im = incremental_mastery_loss_weight * train_incremental_mastery_loss
        train_total_weighted = train_weighted_bce + train_weighted_im
        ...
        if enable_alignment_loss and epoch + 1 >= consistency_rebalance_epoch:
            ...
            ref_corr = global_mastery_corr if global_mastery_corr is not None else 0.0
        ...
        train_history['semantic_trajectory'].append({
            'epoch': epoch + 1,
            # COMMENTED OUT (2025-11-16): Removed correlation metrics from CSV output
        ...
        if val_auc > best_val_auc:
            ...
            # Save best model
            save_dir = f"saved_model/gainakt3exp_{experiment_suffix}"
            ...
            try:
                ...
                with open(repro_metrics_csv, 'a', newline='') as fcsv:
                    ...
                    writer.writerow([
                        epoch + 1,
                        # Overall metrics (combined)
                        f'{train_loss:.6f}', f'{train_auc:.6f}', f'{train_acc:.6f}',
                        f'{val_loss:.6f}', f'{val_auc:.6f}', f'{val_acc:.6f}',
                        # Loss components (unweighted)
                        f'{train_main_loss:.6f}', f'{train_incremental_mastery_loss:.6f}',
                        f'{val_main_loss_avg:.6f}', f'{val_incremental_mastery_loss_avg:.6f}',
                        # Weighted losses (as used in optimization)
                        f'{train_weighted_bce:.6f}', f'{train_weighted_im:.6f}', f'{train_total_weighted:.6f}',
                        f'{val_weighted_bce:.6f}', f'{val_weighted_im:.6f}', f'{val_total_weighted:.6f}',
                        # Loss shares (percentage contribution)
                        f'{bce_loss_share:.6f}', f'{incremental_mastery_loss_share:.6f}',
                        f'{val_bce_share:.6f}', f'{val_im_share:.6f}',
                        # Encoder 1 (Performance Path) metrics
                        f'{train_auc_encoder1:.6f}', f'{train_acc_encoder1:.6f}',
                        f'{val_auc_encoder1:.6f}', f'{val_acc_encoder1:.6f}',
                        # Encoder 2 (Interpretability Path) metrics
                        f'{train_auc_encoder2:.6f}', f'{train_acc_encoder2:.6f}',
                        f'{val_auc_encoder2:.6f}', f'{val_acc_encoder2:.6f}',
                        # Interpretability quality metrics
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Train GainAKT3Exp model with reproducibility hooks.')
    ...
    parser.add_argument('--threshold_temperature', type=float, required=True,
                        help='Temperature parameter for threshold-based sigmoid predictions (adjusted for [0,1] scale)')
    
    # =====================================================================
    # DEPRECATED PARAMETERS (2025-11-16)
    # These parameters are no longer used in GainAKT3Exp dual-encoder architecture.
    # They correspond to features that have been commented out in the code.
    ...
    # DEPRECATED (2025-11-16): intrinsic_gain_attention removed from GainAKT3Exp dual-encoder
    # The following code handled the intrinsic_gain_attention architectural constraint.
    # Now commented out since the feature is deprecated.
