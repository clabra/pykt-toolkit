...
def compute_metrics(y_true, y_pred):
    ...
    if len(y_t) < 2:
        return {"pearson_r": 0, "r2": 0, "mae": 0, "rmse": 0, "count": len(y_t)}
...
def plot_recovery(y_true, y_pred, title, output_path, color='blue', label_prefix=""):
    ...
    sns.regplot(x=y_t_plot, y=y_p_plot, 
                ...
                line_kws={'color': 'red', 'label': f'Linear Fit ({label_prefix})'})
    
    plt.plot([0, 1], [0, 1], transform=plt.gca().transAxes, ls="--", c=".3", label='Theoretical Ideal (y=x)')
    
    metrics = compute_metrics(y_true, y_pred)
    stats_text = (f"Pearson $r = {metrics['pearson_r']:.3f}$\n"
                  f"$R^2 = {metrics['r2']:.3f}$\n"
    ...
    plt.xlabel("Target Oracle BKT Parameters (Skill-level)", fontsize=12)
    plt.ylabel("Proposed Model Estimation", fontsize=12)
    plt.xlim(0, 1)
    plt.ylim(0, 1)
...
def main():
    ...
    pd.DataFrame(skill_metrics).to_csv(os.path.join(args.output_dir, "skill_recovery_metrics.csv"), index=False)
    
    # Generate Plots
    plot_recovery(results["l0_true"], results["l0_probe"], 
                  "Initial Mastery Recovery ($P(L_0)$ Probing)", 
                  os.path.join(args.output_dir, "recovery_l0_probe.png"), color='royalblue', label_prefix="Probe")
    
    plot_recovery(results["t_true"], results["t_probe"], 
                  "Learning Rate Recovery ($P(T)$ Probing)", 
                  os.path.join(args.output_dir, "recovery_t_probe.png"), color='darkviolet', label_prefix="Probe")

    with open(os.path.join(args.output_dir, "recovery_summary.json"), 'w') as f:
