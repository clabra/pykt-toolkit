...
3. **Learning transition**: P(L\_{t+1}) = P(L_t|obs) + (1 - P(L_t|obs)) × P(T)
...
3. **`estimate_bkt_mastery_states(df, model, params, num_skills)`**: Runs forward algorithm to generate mastery trajectories
...
4. **`enforce_monotonicity_skill_wise(targets_dict)`**: Optional smoothing to enforce P(L*t) ≤ P(L*{t+1})
...
- `data/assist2015/bkt_mastery_states.pkl` (201M): Standard version
- `data/assist2015/bkt_mastery_states_mono.pkl` (201M): Monotonic version
...
1. **`load_bkt_parameters(dataset, bkt_path=None)`**: Loads pre-trained BKT parameters

   ```python
   # Returns: bkt_params dict, metadata
   bkt_path = f"data/{dataset}/bkt_mastery_states.pkl"
   ```
...
3. **`compute_bkt_mastery(bkt_params, df)`**: Runs BKT forward inference
...
2. **Computing BKT Correlation**:
   ...
   python examples/compute_bkt_correlation.py \
     ...
     --dataset assist2015 \
     --bkt_path data/assist2015/bkt_mastery_states.pkl
   ```

   - Loads pre-computed **BKT parameters** from `bkt_mastery_states.pkl` (trained on all train+valid students)
...
- **Example**: `bkt_mastery_states.pkl` (201M)
- **Role**: Provides dynamic mastery baselines for correlation analysis
- **Used by**: `compute_bkt_correlation.py` → `load_bkt_parameters()`
...
bkt_mastery = compute_bkt_mastery(bkt_params, df)  # BKT forward inference
...
- `examples/train_bkt.py`: Parameter training via EM
...
- `data/assist2015/bkt_mastery_states.pkl` (201M): Pre-trained BKT parameters
...
- `bibliography/bkt`: relevant papers about the topic
- `bibliography/bkt/bib_bkt.md`: notes about the papers in `bibliography/bkt`
