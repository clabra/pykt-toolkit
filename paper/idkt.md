...
| **ASSIST2009** | -6.40* | -25.75 | **+19.35** | **Positive Selectivity** |

*Note: For ASSIST2009, while absolute fit ($R^2$) is impacted by distribution shifts in the windowed sequence alignment, the **positive selectivity** definitively proves that the model tracks the true theoretical signal significantly better than random noise.*
...
1.  **High Positive Selectivity (+19.35)**: This is the definitive metric for scientific fidelity. It demonstrates that the iDKT embeddings contain substantially more structural information about the true BKT mastery parameters than about random noise. Were the model merely memorizing data patterns (a "Black Box"), it would predict shuffled targets with comparable efficacy, yielding a selectivity near zero. The observed positive delta provides strong empirical support for **Hypothesis 0b (Theoretical Compatibility)**.

2.  **Calibration Divergence (Negative $R^2$)**: The negative absolute goodness-of-fit ($R^2$) for the linear probe indicates that while the *structure* is encoded (as evidenced by the clear clusters in the PCA plots), it is not *linearly mapped* to the BKT scale in a globally stable manner. This divergence is attributed to the "windowing" effect in long sequences: iDKT processes sliced contexts, whereas the reference BKT tracks global state. This introduces "state noise" that a simple linear probe cannot perfectly calibrate without stateful context. However, the presence of geometric structure (PCA) and high selectivity confirms that the latent information is preserved, satisfying the requirement for structural encoding even if direct value-parity requires more complex alignment transform layers.
