...
\subsection{Problem Context and Definition}
...
\subsection{The Accuracy--Interpretability Gap in Knowledge Tracing}
...
\subsection{Proposed Approach: Grounded Transformers}

Consequently, there is a critical need for approaches that achieve true pedagogical interpretability by grounding estimations in trusted, empirically validated principles. To address this, we introduce the \textit{grounded Transformer (gTransformer)}, which implements a novel \textit{Representational Grounding} methodology to bridge the gap between deep learning expressiveness and theoretical alignment. This architecture explicitly anchors the Transformer’s latent representations to the concepts of an intrinsically interpretable reference model, such as BKT. 

By employing an encoder-decoder design to create context vectors that are projected into context-aware parameters, gTransformer maintains the high predictive capacity of attention-based models while driving predictions through the reference model's interpretable logic. This approach provides two practical advantages: it generates actionable parameters (e.g., prior knowledge and learning rates) that are directly meaningful to educators, and it enables context-aware diagnostics that overcome the Markovian limitations of traditional BKT. 
...
Are gTransformer models able to produce interpretable outputs while remaining competitive in terms of predictive performance? What is the trade-off involved?

\item RQ2: \textbf{Grounded Interpretability.}
How can we verify that the parameters used for interpretable outputs are genuinely grounded in theory-based estimations?  

\item RQ3: \textbf{Student-Centred Personalisation.}
How can the captured longitudinal context enhance student-centred personalisation compared to population-based models?
\end{itemize} \label{sec_rq}

The remainder of this paper is structured as follows: \Cref{sec_related} reviews related work; \cref{sec_arch} describes the gTransformer architecture; \cref{sec_experiment} presents the experimental setup; \cref{sec_results} discusses the empirical results; and \cref{sec_conclusions} provides the conclusions. 
...
\end{equation}
These bases are implemented as scalar embeddings with Gaussian initialization centred at the theoretical values.
...
\end{equation}
This design ensures that the model's parameter estimates remain structurally anchored to BKT theory, with the base terms providing pedagogically sound initialization, while contextual projections enable individualised adjustments driven by observed student behavior.
...
\begin{itemize}
    ...
    \item \textbf{NeurIPS 2020 Education Challenge (NIPS34)}: A more recent dataset from the Eedi platform containing student answers to multiple-choice math questions, characterised by subjects trees used for concept mapping \citep{wang2020instructions}.
\end{itemize}
...
Ultimately, gTransformer provides a practical approach for transforming opaque deep learning predictors into actionable interpretable tools. By grounding high-capacity models in established domain theory, this approach offers a path toward student-centred personalization that is both highly accurate and pedagogically interpretable.
