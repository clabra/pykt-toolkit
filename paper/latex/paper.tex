...
\Title{Interpretable Deep Knowledge Tracing via Theory-Grounded Embeddings: Toward Individualized Learning Trajectories}
...
Historically, the development of KT has been characterized by a dichotomy between two distinct epistemological paradigms. The first, which dominated the field for decades, is exemplified by probabilistic graphical models such as Bayesian Knowledge Tracing (BKT) \citep{corbett1994knowledge} and factor analysis methods like the Additive Factor Model (AFM) \citep{cen2006learning}. These traditional approaches are intrinsically interpretable: they model learning as a structured process governed by explicit parameters with clear pedagogical semantics—such as Initial Knowledge ($P(L_0)$), Learning Rate ($P(T)$), Guess ($P(G)$), and Slip ($P(S)$) probabilities. Because the model structure mirrors a theoretical understanding of human learning, educators can inspect these parameters to validate the model's reasoning and trust its diagnostic outputs. However, this transparency comes at the cost of representational rigidity; the simplifying assumptions required by these models (e.g., memorylessness in standard BKT) often limit their predictive power, causing them to struggle with the complex, non-linear, and long-range dependencies inherent in real-world student behavior \citep{twenty_five_years_of_bkt}.

The second paradigm emerged with the advent of Deep Knowledge Tracing (DKT) \citep{piech2015deep}, which shifted the modeling logic from probabilistic inference to high-dimensional representation learning. By leveraging Recurrent Neural Networks (RNNs) and, more recently, Transformer architectures \citep{vaswani2017attention}, DKT models treat student interaction histories as complex temporal sequences. These models have achieved state-of-the-art predictive performance, significantly outperforming classical baselines by automatically extracting latent features from vast amounts of data \citep{abdelrahman2023knowledge}. Yet, this dramatic increase in predictive accuracy has engendered a profound crisis of interpretability. Deep learning models are notoriously opaque ``black boxes,'' where the learned representations are distributed across high-dimensional latent spaces that bear no direct correspondence to pedagogical constructs. In a high-stakes domain such as education—where algorithmic decisions influence learning trajectories, grades, and opportunities—this lack of transparency is unacceptable. A model that predicts a student fails with 90\% probability provides little pedagogical value if it cannot explain \textit{why}, nor can it differentiate between a student who lacks the requisite skill and one who is merely disengaged.

The challenge of interpretability in DKT has recently garnered significant attention, with a recent survey by \citet{bai2024survey} categorizing efforts into two primary modalities: \textit{post-hoc} and \textit{ante-hoc} approaches. Post-hoc methods attempt to explain a trained black-box model using techniques such as Layer-wise Relevance Propagation (LRP), perturbation analysis, or local approximations like LIME. However, as argued by \citet{rudin2019stop}, relying on post-hoc explanations for high-stakes decision-making is fundamentally flawed. These methods produce approximations of the model's decision boundary rather than revealing its true underlying mechanism, often yielding explanations that are unfaithful to the model's actual computation. Furthermore, they require significant technical expertise to interpret, making them inaccessible to the teachers and domain experts who need them most.

Consequently, research has pivoted toward \textit{ante-hoc} or model-intrinsic interpretability, where the model architecture itself is designed to be transparent. In DKT, this has largely manifested through the adoption of attention mechanisms, as seen in models like SAKT \citep{pandey2019self}, SAINT \citep{choi2020towards}, and AKT \citep{ghosh2020context}. While these models often incorporate features derived from educational theories—such as Item Response Theory (IRT) parameters or exercise difficulty embeddings—current approaches remain unsatisfactory for two critical reasons. First, the integration of educational psychology theories has primarily been treated as feature engineering to boost predictive performance (AUC), rather than as a constraint to enforce semantic validity. The resulting latent states, while performance-enhancing, do not necessarily align with the theoretical constructs they supposedly represent. Second, the reliance on attention weights as a proxy for interpretability is epistemologically problematic; attention is a mechanism of relevance, not a construct of causality, and visualizing attention maps often fails to provide a coherent pedagogical narrative explaining the student's knowledge state.

We argue that resolving this tension requires a fundamentally different approach, one that bridges the high capacity of deep learning with the semantic rigor of traditional models. We propose a \textit{Recall and Reconstruct} paradigm of interpretability, which we formally define as \textit{Representational Grounding}. In this framework, interpretability is not an external explanation applied to a model, but a structural property of the model's latent space. Specifically, we posit that an interpretable deep learning model must satisfy three requirements:
\begin{enumerate}
    \item \textbf{State Estimation}: The model must produce explicit estimations of knowledge state evolution.
    \item \textbf{Theoretical Grounding}: These estimations must be linearly recoverable from theoretical principles (e.g., BKT parameters), proving that the deep representation is aligned with established pedagogical semantics.
    \item \textbf{Causal Derivation}: The final performance predictions must be mathematically derived from these grounded estimations, ensuring that the ``explanation'' is the actual cause of the prediction.
\end{enumerate}

Guided by this definition, this paper introduces \textbf{iDKT} (Interpretable Deep Knowledge Tracing), a novel Transformer-based architecture that utilizes Representational Grounding to align its deep latent representations with the educational constructs defined by intrinsically interpretable models, such as BKT. Rather than discarding the deep learning advantage, iDKT decomposes its high-dimensional embeddings into a population-level base (derived from theory) and a student-specific deviation (learned from data), thereby achieving state-of-the-art predictive performance while guaranteeing that its internal states remain pedagogically meaningful. To our knowledge, this is the first work to formalize and validate representational alignment as a rigorous metric for Deep Knowledge Tracing.

A pervasive misconception in machine learning is the existence of an inherent trade-off between predictive accuracy and model interpretability—a belief that complex "black-box" models are necessary to achieve state-of-the-art performance, while interpretable models must sacrifice accuracy for simplicity. \citet{rudin2019stop} argues that this dichotomy is largely a myth, particularly in domains where data possesses significant structure. This perspective is formalized by the \textit{Rashomon Set} argument \citep{liangstatistical, semenova2019study}, which posits that for many real-world problems, the space of models that achieve reasonably high accuracy (the Rashomon set) is voluminous. Given the size of this set, it is statistically probable that it contains a subset of models that are both highly accurate and intrinsically interpretable.

Applying this theoretical framework to Knowledge Tracing, we formulate the hypothesis that the set of high-performing Deep Knowledge Tracing architectures indeed contains solutions that align with pedagogical theory. By imposing specific inductive biases—such as our proposed Representational Grounding—we do not constrain the model's capacity to learn; rather, we navigate the Rashomon set to select a solution that satisfies both predictive optimality and semantic intelligibility. Our work thus aims to explore this frontier, demonstrating that it is possible to recover the rich, pedagogically valid constructs of traditional theory without forfeiting the predictive power of modern deep learning.


To effectively navigate the Rashomon set and achieve an optimal trade-off between predictive performance and interpretability, the choice of the reference model is paramount. A grounding mechanism is only as valid as the theoretical constructs it serves to anchor; if the reference model is pedagogically baseless or conceptually incoherent, the resulting deep representations will be equally flawed, regardless of their fidelity to the reference. We therefore require a reference model that is not merely interpretable, but scientifically sound—one whose parameters map unambiguously to well-established cognitive processes.

For this study, we select Bayesian Knowledge Tracing (BKT) as our anchoring theoretical framework. As detailed in Section \ref{sec_related_bkt}, BKT models learning as a hidden Markov process governed by four explicit parameters: Initial Mastery ($P(L_0)$), the probability a student knows a skill *prior* to practice; Learn Rate ($P(T)$), the probability of transitioning from unlearned to learned states after an opportunity; Guess ($P(G)$), the probability of a correct response despite a lack of mastery; and Slip ($P(S)$), the probability of an incorrect response despite mastery. These parameters provide a causal narrative of student performance that is intuitive to educators and aligned with cognitive science principles.

However, standard BKT suffers from a significant limitation: it typically operates at a population level, estimating a single set of parameters ($L_0, T, G, S$) for all students interacting with a given skill. While Individualized BKT (iBKT) approaches exist \citep{yudelson2013individualized}, they face substantial scalability challenges. Modeling student-specific parameters (e.g., a unique $L_0$ and $T$ for every student-skill pair) leads to extreme data sparsity and the "cold-start" problem, where reliable estimates cannot be formed until a student has generated a lengthy interaction history. Theoretical attempts to individualize BKT often result in models that are computationally expensive and difficult to deploy in real-time environments. iDKT aims to transcend these limitations by using deep learning to infer individualized parameter configurations dynamically, effectively "super-resolving" the population-level insights of BKT into student-specific diagnostics without the need for explicit per-student parameter fitting.

Based on the theoretical framework of Representational Grounding and the Rashomon set argument, this study investigates the following three research hypotheses:

\begin{itemize}
    \item \textbf{Hypothesis 1 (Recoverability)}: It is possible to achieve Theory-Grounded Interpretability in Deep Knowledge Tracing, as defined by our validation framework, by constraining the model's latent space to align with the parameters of a sound reference model like BKT. Specifically, we hypothesize that iDKT's internal states will exhibit high linear recoverability ($R^2 > 0.5$) with BKT parameters.
    \item \textbf{Hypothesis 2 (Pareto Optimality)}: Theory-Grounded Interpretability does not necessitate a loss of predictive accuracy. We hypothesize that by exploring the trade-off via a multi-objective optimization sweep, we can identify a Pareto frontier where iDKT matches or exceeds the AUC of state-of-the-art DKT models while maintaining high representational alignment, effectively locating an interpretable solution within the Rashomon set.
    \item \textbf{Hypothesis 3 (Granularity)}: The grounded representations in iDKT are not merely static copies of the reference model but capture student-specific nuances. We hypothesize that the deviations learned by iDKT ($l_c, t_c$) represent valid, granular estimations of student-specific Initial Mastery and Learning Rates, enabling individualized diagnosis and pacing that is more sensitive to behavioral heterogeneity than the population-level reference.
\end{itemize}

\subsection{Probing Methods}

\textit{Linear classifier probes} \citep{alain2016understanding} have emerged as a robust framework for investigating the information encoded in the intermediate layers of neural networks. Originally introduced to analyze representations in deep vision and language models, probing involves training a simple diagnostic classifier to predict a specific property or construct—such as a student's latent mastery state—from a frozen embedding. The fundamental intuition is that if a construct can be accurately predicted by a simple model, it must be explicitly encoded and accessible within the deep model's representational space. This methodology is particularly relevant in the context of recent surveys on Large Language Models (LLMs) and foundation models, where understanding the internal mechanics and \textit{representation engineering} has become a central research frontier \citep{zhao2024explainability, bartoszcze2025representation}.

The state-of-the-art in probing emphasizes the importance of utilizing low-capacity models, typically linear or logistic regressions, to ensure that the probe is merely extracting existing information rather than learning the task itself through its own parameters. This distinction is critical in our context, where we seek to verify that the deep model has internalized valid constructs of pedagogical theories as part of its internal reasoning process. Recent advances have expanded this methodology to include \textit{structural probes}, which evaluate whether relational properties between concepts are preserved within the latent manifold. A significant challenge in interpreting probe results is the potential for false positives, where a probe achieves high accuracy by exploiting statistical artifacts rather than meaningful structure. To address this, current research highlights the necessity of methodological rigor through the use of \textit{selectivity} metrics and \textit{control tasks} . By comparing the probe's performance on the target construct against its performance on randomized or nonsensical data, researchers can quantify the extent to which the deep model genuinely represents the theory \citep{hewitt2019designing,belinkov2022probing}. 
