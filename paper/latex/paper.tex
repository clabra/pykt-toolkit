...
\bottomrule
\end{tabular}
\caption{Parameter values used for gTransformer architecture and training}
...
\midrule
gTransformer & \textbf{0.7838} & 0.7081 & 0.8306 & 0.8066 & 0.7908 \\
...
Theory-based interpretability can be validated through a dual approach: 
...
Together, these demonstrate that grounding constraints actively shape the model's internal structure, not merely correlate with outputs, while maintaining competitive predictive performance. This dual hypothesis addresses a fundamental challenge in educational AI: whether neural models can internalize pedagogical theory rather than learning arbitrary features that happen to predict well, and whether such interpretability can be achieved without sacrificing the representational power that makes deep learning effective.

To validate these hypotheses, we employ \textit{diagnostic probing with control tasks} \citep{hewitt2019designing} to test structural encoding and \textit{parameter correlation analysis} to test semantic alignment. For structural validation, we train linear probes to recover BKT parameters from the model's hidden representations, comparing performance on true parameters against randomly shuffled controls to measure selectivity ($\Delta R^2 = R^2_{\text{true}} - R^2_{\text{control}}$). The rationale is that high selectivity ($> 0.5$) proves BKT constructs are the primary organizing axis rather than incidental patterns—if probes perform equally well on shuffled targets, the representations would contain no genuine structural encoding. For semantic validation, we compute Pearson correlations between probe-extracted parameters and population-level BKT priors fitted on training data, where high correlation ($r > 0.7$) proves parameters have pedagogically meaningful values reflecting established educational theory. We complement these with performance comparison against unconstrained baselines to verify that interpretability is achieved without accuracy sacrifice (gap $< 0.5$ percentage points), and comparison between active grounding (constraints enforced during training) versus post-hoc probing (probes fitted to frozen baseline representations) to confirm that interpretability must be designed into the architecture rather than retrofitted. The following subsections present the experimental evidence obtained. 

\subsubsection{Structural Alignment via Diagnostic Probing} \label{sec_structural_alignment}

To verify whether gTransformer's latent representations genuinely internalize pedagogical concepts, we employ diagnostic probing with control tasks \citep{hewitt2019designing,belinkov2022probing}. We train linear Ridge regression probes ($\alpha=1.0$) to recover BKT constructs---initial mastery $p_{L_0}$ and learning rate $p_{T}$---from the final contextualized hidden representations $z_t$. 

\textbf{Methodology.} To distinguish genuine structural encoding from spurious correlations, we measure \textit{Selectivity} as $\Delta R^2 = R^2_{\text{true}} - R^2_{\text{control}}$, comparing probe performance on true BKT targets against a control task with randomly shuffled target values. This tests whether probes exploit genuine BKT structure versus arbitrary patterns. We use an 80/20 train-test split and measure fidelity via $R^2$ and Pearson correlation $r$.  

\textbf{Results.} \Cref{tab_probing} summarizes the diagnostic probing results on ASSIST2009. Both BKT constructs demonstrate robust structural encoding:

\begin{itemize}
    \item \textbf{Initial Mastery ($L_0$)}: Achieves fidelity $R^2 = 0.487$ with Pearson $r = 0.698$, indicating strong linear recoverability. Selectivity $\Delta R^2 = 0.507$ substantially exceeds the threshold of 0.5, proving $L_0$ is a dominant organizing principle in the latent space rather than an incidental pattern. The control task achieves negative $R^2 = -0.020$, confirming probes cannot recover shuffled targets.
    
    \item \textbf{Learning Rate ($T$)}: Shows even stronger structural encoding with fidelity $R^2 = 0.566$ and Pearson $r = 0.753$. Selectivity reaches $\Delta R^2 = 0.607$, the highest among both constructs, with control $R^2 = -0.041$. This superior selectivity suggests learning rate trajectories impose more distinctive constraints on latent geometry than initial mastery.
\end{itemize}
...
\toprule
\textbf{Construct} & \textbf{Fidelity ($R^2$)} & \textbf{Pearson ($r$)} & \textbf{Selectivity ($\Delta R^2$)} & \textbf{Control ($R^2$)} \\
\midrule
Initial Mastery ($L_0$) & 0.487 & 0.698 & \textbf{0.507} & $-0.020$ \\
Learning Rate ($T$) & 0.566 & 0.753 & \textbf{0.607} & $-0.041$ \\
\bottomrule
\end{tabularx}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{img/h1_structural_fidelity_l0_fold_0_481134.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{img/h1_structural_fidelity_t_fold_0_481134.png}
...
\subsubsection{Semantic Grounding and Convergent Validity} \label{sec_semantic_grounding}

Beyond structural recovery, we evaluate the semantic alignment between the model's projected parameters and theoretical priors. For each student interaction, we analyze the distribution of individualized grounded parameters $p_{L_0,t}$ and $p_{T,t}$. We measure \textit{Representational Convergence} ($\mathcal{I}_1$) as the Pearson correlation between these grounded parameters and the population-level BKT priors ($\ell_{L0}$, $\ell_T$).

Results across multiple datasets (\Cref{tab_interpretability}) show consistently high correlations ($\mathcal{I}_1 > 0.82$), indicating that gTransformer's individualization mechanism preserves the semantic meaning of the underlying theory. The model does not "repurpose" these parameters for black-box optimization but rather refines them within the pedagogical bounds established by the reference model.

\subsubsection{Functional Alignment and Predictor Equivalence} \label{sec_functional_alignment}

Finally, we assess whether gTransformer can achieve \textit{Predictor Equivalence} ($\mathcal{I}_2$)---the ability of an interpretable sub-component to emulate the full neural model's behavior. We pass the grounded parameters $\{p_{L_0,t}, p_{T,t}\}$ through the standard BKT transition and emission equations to generate "interpretable" predictions $\hat{y}_{ref}$. We then measure the functional alignment $\mathcal{I}_2$ as the correlation between the neural supervised predictions $\hat{y}_{sup}$ and these theory-derived predictions $\hat{y}_{ref}$.

As shown in \Cref{tab_interpretability}, gTransformer achieves $\mathcal{I}_2$ values exceeding 0.95 across all benchmarks. This nearly perfect alignment demonstrates that the complex attention-driven logic of the Transformer is successfully distilled into the interpretable BKT framework, achieving representational transparency without accuracy loss.

\begin{table}[htbp]
\centering
\caption{Cross-dataset Interpretability Metrics. $\mathcal{I}_1$: Representational Convergence (mean correlation with theory); $\mathcal{I}_2$: Functional Alignment (correlation with neural predictions). \label{tab_interpretability}}
\footnotesize
\begin{tabularx}{\textwidth}{lccccc}
\toprule
\textbf{Metric} & \textbf{AS2009} & \textbf{AS2015} & \textbf{AL2005} & \textbf{BDG2006} & \textbf{NIPS34} \\
\midrule
