...
\added[id=rev]{To validate gTransformer's capabilities, we employ a multi-faceted evaluation strategy. The grounded (non-ablated) gTransformer variant generates two outputs: (1) supervised predictions from the standard output head {($p_{\text{sup}2}$)} and (2) interpretable predictions {($p_{\text{ref}}$)} derived from the grounded parameters. To evaluate the performance trade-off, we use also (3) supervised predictions from the ablated configuration {($p_{\text{sup}1}$)}, which establishes the maximum predictive capacity in the absence of interpretability constraints, and (4) predictions from the BKT model {($p_{\text{bkt}}$)} using population-level parameters. We quantify {\textit{Interpretability Drop}} as {($p_{\text{sup}1} - p_{\text{sup}2}$)}, the expected reduction in supervised predictive power caused by the structural constraints required to align the latent space with theoretical constructs. The {\textit{Interpretability Cost}} is measured as {($p_{\text{sup}2} - p_{\text{ref}}$)}, representing the sacrifice in accuracy required to achieve intrinsic pedagogical transparency. Conversely, the {\textit{Interpretability Gain}}, defined as {($p_{\text{ref}} - p_{\text{bkt}}$)}, represents the performance benefit of our grounded approach over the interpretable BKT baseline.}

\added[id=rev]{For {\textit{grounded interpretability}} (RQ2), we establish a triple validation protocol consisting of the following hypotheses:}
\begin{itemize}
