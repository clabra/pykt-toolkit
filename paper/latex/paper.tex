...
Table \ref{tab_interpretability_baseline} reports the baseline interpretability metrics for the unconstrained iDKT model ($\lambda_{ref}=0$).

\begin{table}[H]
\caption{Interpretability Alignment Metrics for unconstrained iDKT ($\lambda_{ref}=0$). \label{tab_interpretability_baseline}}
\footnotesize
...
\subsection{Discussion of Alignment Results} \label{sec_alignment_discussion}
...
A key contribution of this paper is the systematic exploration of the trade-off between predictive accuracy and interpretability. By modulating the grounding weight $\lambda_{ref}$ in Equation 7, we identify the Pareto frontier of the model. This allows us to determine the points where theoretical grounding acts as a beneficial regularizer that improves generalization, and the point where excessive adherence to the reference model begins to degrade predictive power.
...
\subsection{Granularity of Individualization} \label{sec_gran_results}
