...
\begin{enumerate}
    ...
    \item \textit{Prediction and Interpretability Heads}: the encoder and decoder components send the context vector ($z_t$) to the prediction head ($h_{pred}$) and the interpretability head ($h_{int}$). The prediction head outputs the probability of the correct answer ($p_{pred}$), while the interpretability head outputs the estimated proficiency ($p_{know}$) and rate ($p_{rate}$) parameters.
    \item \textit{Loss Functions}: optimization is driven by a multi-objective loss pipeline. While the prediction head is supervised by $L_{pred}$, the interpretability head is guided by $L_{int}$, which penalizes deviations between the inferred parameters ($p_{know}, p_{rate}$) and the BKT grounding values. This mechanism operationalizes \textit{Representational Grounding}, steering the system to learn latent context representations that remain both predictive and semantically aligned with the reference model.
\end{enumerate}

\begin{figure}[!htbp]
    \centering
    \includegraphics[height=0.6\textheight, width=\textwidth, keepaspectratio]{img/arch.png}
    \caption{The diagram illustrates the layers of the iDKT Architecture: (1) \textit{Input Layer}: data enriched with BKT reference data ($p_{BKT}$ and parameters); (2) \textit{Embeddings}: provides the foundational material through individualized transformations ($x', y'$); (3) \textit{Attention Heads}: encoder-decoder transformer synthesizes interaction history and current task into a contextualized representation ($z_t$); (4) \textit{Prediction and Interpretability Heads}: provide both performance probabilities ($p_{pred}$) and pedagogical diagnostics ($p_{know}, p_{rate}$); (5) \textit{Loss Functions}: implement a multi-objective pipeline ($L_{pred}, L_{int}$) to ensure representational grounding; and (6) \textit{Diagnostic Probing Validation}: verifies the structural preservation of the pedagogical constructs through linear probes.}
    \label{idkt_arch}
\end{figure}
