...
\subsection{Individualized Learning Trajectories}
...
The core architecture of iDKT, illustrated in Figure~\ref{idkt_arch}, extends the standard Transformer framework by incorporating specialized components for Representational Grounding within the embedding layers and the multi-objective loss pipeline. The information flow consists of five main stages:
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[height=0.8\textheight, width=\textwidth, keepaspectratio]{img/idkt_arch_d2.png}
    \caption{The iDKT Architecture. The diagram illustrates the complete pipeline: (1) \textbf{Input Layer} with BKT reference data, (2) \textbf{Representational Grounding} where input embeddings incorporate personalized initial knowledge ($l_c$) and learning rate ($t_c$) derived from BKT parameters, (3) \textbf{Encoder-Decoder Transformer} processing interaction history and tasks, (4) \textbf{Prediction} via concatenated representations and MLP, (5) \textbf{Multi-objective Loss Functions} for grounding, and (6) \textbf{Diagnostic Probing Validation} that validates preservation of pedagogical constructs through linear probes with control tasks.}
    \label{idkt_arch}
\end{figure}
...
\section{Empirical Results} \label{sec_results}
