...
In standard educational datasets such as ASSIST2009 and ASSIST2015, student interactions are recorded at the level of specific questions, each of which is associated with one or more underlying pedagogical concepts. This structure establishes a natural hierarchy: a concept represents an abstract skill (e.g., "Pythagorean Theorem") whose learning is operationalized through a diverse set of questions. While all questions belonging to a concept share the same semantic core, they differ in their specific manifestations—most notably in their intrinsic difficulty or complexity. Therefore, an effective representation must capture both the shared identity of the concept and the unique deviation of the specific question.

In Transformer-based models we can operationalize this principle representing the tasks as embedding vectors: 

$$x' = c + u \cdot d$$

In this formulation, $c$ acts as the \textit{Concept Anchor}, a vector representing the invariant semantic identity of the concept while the vector $d$ represents the learnable \textit{Question Variation Axis}, defining the specific direction of "increasing challenge". The scalar $u$ serves as the \textit{Relational Magnitude}, representing the question's specific relative difficulty compared to other questions involving the same concept. Consequently, rather than encode arbitrary embeddings for every question, we encode the vector sum of two distinct components with clear semantic meaning: a base \textit{concept identity} and a \textit{difficulty shift}. 
...
Previously, to get individualized values for these parameters, we decompose them into population-level bases and student-specific deviations: 
...
The final input to the encoder and decoder components of the Transformer consists of: 
\begin{equation}
...
The rationale for using difference for tasks ($x'$) and sum for history ($y'$) is rooted in their distinct semantic roles:
\begin{itemize}
    \item Individualized Task represents the "Challenge": $x' = \text{Difficulty} - \text{Capability}$ codifies the \textit{Net Challenge} for this specific student. If a student is very capable (i.e. has a high $l_c$), the effective difficulty of the task should be lower. The model asks: "How much difficulty remains after accounting for the student's ability?" This constitutes a kind of relational logic where difficulty is relative to the student.
    \item Interaction History represents the "Evidence": $y' = \text{Interaction} + \text{Velocity}$ codifies the \textit{Accumulated Momentum} or evidence. A correct answer from a fast learner (i.e. with a high $t_s$) provides more signal than one from a slow learner. The history encoder accumulates these evidences, adding the student's learning speed to the signal because high velocity implies faster mastery acquisition. It asks: "What is the total learning value derived from this interaction given the student's speed?"
