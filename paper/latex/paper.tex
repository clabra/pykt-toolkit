...
\bottomrule
\end{tabularx}
\caption{Semantic Alignment results for AS2009 (H1.2). Spearman $\rho$ measures monotonic relationship preservation between grounded parameters and BKT theoretical priors. Comparison with probe-based plots quantifies the degree of context-aware individualization (Gap $\Delta$). Moderate-to-strong correlations confirm pedagogical semantics are preserved through neural transformation while enabling student-specific refinement. \label{tab_semantic_alignment}}
\end{table}
...
To quantify the performance trade-off between predictive accuracy and interpretability, we compare three prediction sources across five educational datasets using a gTransformer architecture with ablation=none, $d_{\text{model}}=64$, 4 blocks and 4 attention heads. The model produces two types of predictions: (1) grounded neural predictions ($p_{\text{sup}2}$) from the standard output layer, maximizing accuracy without interpretability constraints, and (2) interpretable BKT-logic predictions ($p_{\text{ref}}$) derived from student-specific grounded parameters ($p_{L_0}$, $p_T$) extracted from the model's theoretical grounding layer, following classical BKT inference logic. For comparison, we include (3) the classical BKT baseline ($p_{\text{bkt}}$) from a separate traditional BKT model using population-level parameters without neural individualization. This baseline is omitted for AS2015 because the test data does not include the question column required for standardized question-level evaluation. Table~\ref{tab:tradeoff} presents the test AUC results and the associated cost of interpretability ($p_{\text{sup}1} - p_{\text{ref}}$).
...
\toprule
\textbf{Dataset} & $p_{\text{sup}1}$ & $p_{\text{sup}2}$ & $p_{\text{ref}}$ & $p_{\text{bkt}}$ & \textbf{Pred. Cost (\%)} & \textbf{Int. Gain (\%)} \\
\midrule
AS2009 & 0.7831 & 0.7824 & 0.6733 & 0.6097 & 0.1098 (14.0\%) & 0.0636 (10.4\%) \\
AS2015 & 0.7078 & 0.7073 & 0.6545 & --- & 0.0533 (7.5\%) & --- \\
AL2005 & 0.8240 & 0.8219 & 0.7361 & 0.7215 & 0.0879 (10.7\%) & 0.0146 (2.0\%) \\
Bridge2006 & 0.8148 & 0.8120 & 0.7025 & 0.6756 & 0.1123 (13.8\%) & 0.0269 (4.0\%) \\
NIPS34 & 0.8006 & 0.7991 & 0.6843 & 0.5729 & 0.1163 (14.5\%) & 0.1114 (19.4\%) \\
\midrule
\textbf{Mean} & 0.7861 & 0.7845 & 0.6901 & 0.6449 & 0.0959 (12.1\%) & 0.0541 (9.0\%) \\
\bottomrule
\end{tabular}
\caption{Accuracy--Interpretability trade-off across datasets with gTransformer. The table shows AUC scores for the ablated neural baseline ($p_{\text{sup}1}$), grounded neural predictions ($p_{\text{sup}2}$), interpretable BKT-logic predictions ($p_{\text{ref}}$), and classical BKT baseline ($p_{\text{bkt}}$). \textit{Pred. Cost} represents the performance difference between $p_{\text{sup}1}$ and $p_{\text{ref}}$, while \textit{Int. Gain} measures the predictive performance gained by using gTransformer compared to a traditional BKT model.}
\label{tab:tradeoff}
\end{table}


The empirical results confirm that gTransformer successfully bridges the gap between deep learning performance and theoretical interpretability. As shown in Table~\ref{tab:tradeoff}, the average \textit{cost of interpretability} is 0.0959 AUC points (12.1\% relative reduction), with the gap remaining remarkably consistent (7.5\% to 14.5\%) across all datasets. This validates our grounding architecture's ability to maintain high predictive power while strictly adhering to BKT causal logic. The gain from neural individualization is general: interpretable neural predictions outperform classical BKT in every comparable dataset, with improvements ranging from +2.0\% (Algebra 2005) to +19.4\% (NIPS Task 3/4). This confirms that capturing longitudinal behavioral patterns through individualized parameters provides a fundamental advantage over population-level models, even when constrained by interpretable logic. 

The results reveal two insights regarding the accuracy--interpretability trade-off:

\begin{itemize}
    \item \textbf{Consistent Low-to-Moderate Cost of Interpretability.} The average cost of interpretability across datasets is 0.0959 AUC points (12.1\% relative reduction), demonstrating that theory-grounded interpretability can be achieved with remarkably low performance degradation. The cost is highly consistent across all datasets, ranging from a minimum of 7.5\% (ASSISTments 2015) to a maximum of 14.5\% (NIPS Task 3/4). This stability suggests that the neuro-symbolic framework is robust to varying dataset characteristics. 
