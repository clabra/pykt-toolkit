...
Are gTransformer models able to produce interpretable outputs while achieving state-of-the-art predictive performance? What is the trade-off between predictive performance  and interpretability?
...
\caption{Semantic Alignment for Initial Mastery $P_{L_0}$. Plot showing weak monotonic relationship preservation (Spearman $\rho = 0.360$, MAE = $0.168$) between BKT theoretical priors and grounded parameters after neural transformation. Each bubble represents an aggregate of test interactions, with size encoding sample density. The substantial scatter indicates strong student-specific individualization, with the model prioritizing context-aware adaptation over strict adherence to population priors. \label{fig_semantic_alignment_l0}}
\vspace{12pt}
\includegraphics[width=0.70\textwidth]{img/h12_recovery_t_grounded_893468.png}
\caption{Semantic Alignment for Learning Rate $P_{T}$. Plot demonstrating moderate monotonic relationship preservation (Spearman $\rho = 0.544$, MAE = $0.092$) between BKT theoretical priors and grounded parameters. The clustering around the diagonal across the full $P_T$ range indicates the model preserves pedagogical understanding of practice effects through neural processing while enabling context-aware refinement. \label{fig_semantic_alignment_t}}
\end{figure}
...
The students were classified into four learning situations based on their historical learning parameters. Figure~\ref{fig:cluster_placement_pacing_contextual_893468.png} shows students classified in the four quadrants for AS2009 dataset:
\begin{itemize}
