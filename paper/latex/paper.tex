...
The resulting Pareto curves are visualized in Figures \ref{fig_pareto_as2015} and \ref{fig_pareto_as2009}. We observe that theoretical guidance serves as a powerful regularizer; even a small weight ($\lambda=0.10$) causes a massive leap in interpretability with minimal impact on accuracy. However, excessive grounding leads to a collapse in denser datasets like AS2009, where the model can no longer reconcile complex behavioral patterns with simple pedagogical assumptions.
...
\begin{tabularx}{\textwidth}{lccc}
\toprule
\textbf{$\lambda_{ref}$} & \textbf{Test AUC} & \textbf{Composite $\bar{I}$} & \textbf{Interpretation} \\
\midrule
...
\caption{iDKT Pareto Frontier for ASSISTments 2009. The curve shows a non-monotonic relationship where interpretability peaks at $\lambda \approx 0.3$, followed by a \textit{grounding collapse} as the model fails to reconcile excessive constraints. \label{fig_pareto_as2009}}
\end{figure}
...
In the following sections, we analyze how the architectural design of iDKT enables high-granularity diagnostics through three distinct perspectives: the statistical variance of estimated student parameters, the resilience of mastery estimations against behavioral noise, and the enabling of individualized adaptive pacing.

\subsubsection{Contextualization} \label{sec_longitudinal}

High-granularity enables the estimation of mastery trajectories by leveraging a student's cross-curricular historical context. 

This capability is illustrated in Figure \ref{fig_mastery_866}, which contrasts the iDKT and BKT estimations for a high-velocity learner. While the BKT baseline fails to account for the student's broader interaction history---thereby penalizing each individual failure with a significant drop in mastery probability---iDKT maintains a stable and resilient diagnostic trajectory. By leveraging the longitudinal context, iDKT is able to contextualize the failures in a more efficient manner. This inherent capacity of Transformer-based architectures, and iDKT in particular, to encode longitudinal context is a key enabler for supporting individualized learning paths. 

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{img/idkt_mastery_866.png}
\caption{Comparison of two mastery trajectories estimated by iDKT and BKT that shows how the iDKT trajectory is more stable and resilient against behavioral noise. \label{fig_mastery_866}} 
\end{figure} 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{img/volume_as2015.png}\\
{\small (b) ASSISTments 2015}
\includegraphics[width=0.9\textwidth]{img/volume_as2009.png}\\
{\small (a) ASSISTments 2009}\\[10pt]
