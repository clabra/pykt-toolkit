...
The core architecture, illustrated in Figure~\ref{fig_arch}, consists of these components:
\begin{enumerate}
    \item \textbf{Input Stage}: Receives tuples of $(q_t, c_t, r_t, uid_t, pid_t)$, mapping raw IDs to their respective theoretical and learned components.
    \item \textbf{Embeddings Stage}: Processes IDs through the Representational Grounding layer to generate individualized task ($x'_t$) and interaction ($y'_t$) embeddings.
    \item \textbf{Encoder Stage}: A context-aware Transformer encoder that extracts local and global patterns from the sequence of past individualized interactions $y'_{1:t-1}$.
    \item \textbf{Decoder Stage}: The Knowledge Retriever, which uses cross-attention between the current task $x'_t$ and the encoded context to estimate the context-aware knowledge state $\hat{x}_t$.
    \item \textbf{Attention Heads}: Multi-scale monotonic heads that apply distance-based decay parameters $\gamma_h$ to control the model's temporal resolution.
    \item \textbf{Output Stage}: A 3-layer MLP that yields the final performance prediction $p_{iDKT}$ and monitoring proxies for mastery alignment.
\end{enumerate}

\begin{figure}[H]
    \centering
    % [Insert iDKT Architecture Diagram here]
...
\subsection{Representational Grounding and Embeddings} \label{sec_emb}

To achieve interpretability-by-design, iDKT replaces standard learned embeddings with what we term \textit{Grounded Embeddings}. We adopt a relational differential logic (Archetype 1) where latent factors are formally decomposed into population-level bases and student-specific deviations.

\subsubsection{Theoretical Foundations}
The model defines a set of population-level primitives that serve as the "ground truth" coordinate system for the latent space:
\begin{itemize}
    \item \textbf{Concept Base ($c_{c_t}$)}: The static vector representation of concept $c_t$, capturing its semantic identity within the curriculum.
    \item \textbf{Interaction Base ($e_{c_t,r_t}$)}: A combined representation of concept $c_t$ and the binary outcome $r_t$ (correct/incorrect).
    \item \textbf{Instructional Prior ($L0_{skill}$)}: A skill-specific embedding anchored to the theoretical BKT initial mastery parameter $L0$. 
    \item \textbf{Learning Velocity Base ($T_{skill}$)}: A skill-specific embedding anchored to the BKT transition probability $T$.
\end{itemize}

\subsubsection{Individualization via Rasch Logic}
To capture student-level and item-level heterogeneity, iDKT introduces three learnable scalars that modulate the theoretical bases:
\begin{itemize}
    \item $u_q$ (\textbf{Problem Difficulty}): A scalar corresponding to the problem ID ($q$), representing its relative challenge compared to others of the same skill.
    \item $k_c$ (\textbf{Knowledge Gap}): A student-specific scalar ($uid$) identifying deviations from the skill's instructional prior.
    \item $v_s$ (\textbf{Learning Speed}): A student-specific scalar identifying individual acquisition momentum.
\end{itemize}

These scalars operate along dedicated \textbf{variation axes}, which are unit vectors defining the semantic direction of adjustment in $d$-dimensional space:
\begin{itemize}
    \item $d_{c_t}$ (\textbf{Question Variation Axis}): Defines how problem difficulty $u_q$ shifts the challenge of concept $c_t$.
    \item $f_{c_t,r_t}$ (\textbf{Interaction Variation Axis}): Defines how item difficulty $u_q$ affects the representation of the interaction history.
    \item $d_c$ (\textbf{Knowledge Axis}): Defines the dimension of prior knowledge variance.
    \item $d_s$ (\textbf{Velocity Axis}): Defines the dimension of learning velocity variance.
\end{itemize}
\begin{equation}
    l_c = L0_{skill} + k_c \cdot d_c \quad \text{(Personalized Initial Knowledge)}
\end{equation}
\begin{equation}
    t_s = T_{skill} + v_s \cdot d_s \quad \text{(Personalized Learning Velocity)}
\end{equation}

\subsubsection{Fusion Layer}
The final input to the Transformer core consists of the \textit{Individualized Task} ($x'_t$) and the \textit{Individualized Interaction History} ($y'_t$):
\begin{equation}
...
This formulation implements what we term \textbf{Relational Differential Fusion} (Archetype 1). By hard-coding the subtraction for current tasks and addition for history, we enforce a \textit{Relational Inductive Bias} ($\text{Challenge} - \text{Capability}$). This ensures that the attention mechanism operates on semantically anchored residuals—specifically, the "Residual Challenge" for current tasks—rather than opaque latent features. This transformation from predictive pattern-matching to differential evaluation allows the model to act as a sensitive diagnostic instrument: when a student with high perceived competence fails a mathematically "easy" task, the resulting gradient spike identifies a clear pedagogical anomaly that would be averaged out in black-box models.

\subsection{Individualized Student Profiling} \label{sec_profiling}

Standard BKT models assume that pedagogical parameters like Initial Mastery ($P(L_0)$) and Learning Rate ($T$) are constant for every student within a skill. iDKT leverages its Transformer backbone to individualize these parameters, generating a \textbf{Multidimensional Learner Profile} that summarizes student capability ($v_s$) and learning efficiency ($T_s$) alongside traditional mastery states.

\subsubsection{Personalized Student Velocity ($T_s$)}
While standard BKT assigns a single $T$ per skill, iDKT estimates a \textbf{Student Velocity} that varies based on individual learning pace:
\begin{itemize}
    \item \textbf{Concept}: Instead of a global $T_{skill}$, we implement a velocity component that receives latent student history.
    \item \textbf{Benefit}: This distinguishes between "fast" and "slow" learners. Educators can identify students with low velocity (learning plateaus) who may require different instructional interventions, regardless of their current mastery levels.
\end{itemize}

\subsubsection{Individualized Initial Knowledge ($P(L_0)$)}
Standard BKT assumes all students start at the same $P(L_0)$ when they first encounter a skill. We individualize this using a \textbf{Student Capability Baseline} ($v_s$) and \textbf{Knowledge Gap} ($k_c$):
\begin{itemize}
    \item \textbf{Mechanism}: A learnable embedding `student\_param` ($v_s \in \mathbb{R}^1$) is associated with each user ID. The initial mastery projection is calculated as: $L_{0}^{(s,c)} = \text{Sigmoid}(\text{Proj}(c) + v_s)$.
    \item \textbf{Benefit}: This allows the model to account for unobserved prior knowledge preventing misjudgment of a student's inherent ability simply because they have not yet practiced a specific topic on the platform.
\end{itemize}

\subsection{Informed Individualization Framework} \label{sec_framework}

The "Informed Individualization" framework has been implemented across the model architecture, training pipeline, and evaluation suite in four distinct phases:

\paragraph{Phase 1: Model Architecture Optimization}
We introduced a student-level capability embedding and redesigned the output heads to incorporate individual traits. The forward pass now processes `uid\_data` to extract $v_s$ and combine it with concept/latent features for individualized parameter projection.

\paragraph{Phase 2: Training Pipeline Integration}
The training script supports the individualized flow and enforces theoretical priors via regularization:
\begin{itemize}
    \item \textbf{Student Regularization ($L_{student}$)}: A new Gaussian prior $L_{reg\_student} = \lambda_{s} \sum v_s^2$ ensures that learned student traits are centered.
    \item \textbf{Dynamic Lambda Calibration}: The "Warm-up Calibration" handles the individualized heads, ensuring stable signal sharing from epoch 1.
\end{itemize}

\paragraph{Phase 3: Individualized Evaluation}
The evaluation suite quantifies the impact of individualization:
\begin{itemize}
    \item \textbf{Capability Profiling}: Learned $v_s$ traits are exported to analyze their correlation with student historical performance.
    \item \textbf{Roster Export}: Longitudinal tracking of mastery states for all students and skills is supported via `IDKTRoster`.
\end{itemize}

\paragraph{Phase 4: Reproducibility \& Documentation}
Parameters are registered in `configs/parameter\_default.json` and architectural changes are tracked via comprehensive documentation updates.

\subsection{Multi-Scale Monotonic Attention} \label{sec_attn}

The iDKT model employs a monotonic attention mechanism to prioritize relevant historical interactions. The attention score $s_{t,\tau}$ between the current task at time $t$ and a past interaction at time $\tau$ is modulated by an exponential temporal decay:
\begin{equation}
    s_{t,\tau} = \frac{q_t^\top k_\tau}{\sqrt{D_k}} \cdot \exp(-\theta \cdot d(t, \tau))
\end{equation}
where $\theta > 0$ is a learnable decay rate and $d(t, \tau)$ is a context-aware distance measure. 

Crucially, iDKT utilizes \textbf{Multi-Head Attention} where each head $h$ possesses a unique decay parameter $\gamma_h$. This allows the model to simultaneously track dynamics at multiple scales: heads with large $\gamma$ focus on short-term mastery acquisition (recent repetitions), while heads with small $\gamma$ monitor long-term concept retention and foundational knowledge.
...
\subsection{The Knowledge Retriever} \label{sec_retriever}

The knowledge retriever consists of $2N$ stacked transformer blocks with an alternating layer pattern:
\begin{itemize}
    \item \textbf{Odd Layers}: Perform causal self-attention on the individualized question sequence ($x'$).
    \item \textbf{Even Layers}: Perform cross-attention between the question sequence (queries) and the encoded interaction history $\hat{y}$ (keys and values).
\end{itemize}
This dual-stream approach allows the model to differentiate between the structure of the curriculum (questions) and the student's actual performance history, outputting a context-aware knowledge state $\hat{x}_t$ that reflects both current intent and historical context.
...
\subsection{Response Prediction} \label{sec_prediction}

The context-aware knowledge state $\hat{x}_t$ from the retriever is concatenated with the current individualized task representation $x'_t$ to form the input to the prediction head. This head is implemented as a multi-layer perceptron (MLP) with the following structure:
\begin{enumerate}
    \item Dense layer: $[2 \cdot d_{model} \to 512]$ with ReLU activation.
    \item Dense layer: $[512 \to 256]$ with ReLU activation.
    \item Output layer: $[256 \to 1]$ with a Sigmoid activation function to estimate $\hat{y}_t \in [0, 1]$.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{$H_3$: Discriminant Validity (Construct Distinctness)}: Verifies that Knowledge Gap ($k_c$) and Learning Velocity ($v_s$) are not perfectly collinear, proving the model can distinguish between prior knowledge and acquisition momentum.
