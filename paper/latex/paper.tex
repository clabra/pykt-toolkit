...
\begin{itemize}
    \item \textbf{Hypothesis 1 (Semantic Alignment)}: Through representational grounding, the internal states of a high-capacity Transformer model can be aligned with pedagogically meaningful constructs. We hypothesize that iDKT will achieve Semantic Alignment by exhibiting high linear recoverability with respect to BKT parameters across diverse educational datasets.
    \item \textbf{Hypothesis 2 (Performance-Interpretability Trade-off)}: The pursuit of intrinsic interpretability does not imply a significant degradation in predictive performance. We hypothesize that through a systematic Pareto analysis, we can identify configurations that preserve near-optimal accuracy while achieving high semantic alignment. Furthermore, we posit that in certain contexts, the inductive bias introduced by pedagogical grounding can act as a synergistic regularizer, actually enhancing the model's predictive accuracy.
