...
\midrule
\textbf{Mean} & 0.7861 & 0.7845 & 0.7390 & 0.6449 & 0.0016 & 0.0471 (5.9\%) & 0.1053 (17.0\%) \\
\bottomrule
\end{tabular}
...
\begin{itemize}
    \item \textbf{Low Cost of Interpretability.} This cost can be analyzed through two distinct metrics: 1) We define the \textit{Interpretability Drop} (\textit{Int. Drop}) as the impact of grounding constraints ($\mathcal{L}_{\text{ref}}$ and $\mathcal{L}_{\text{probe}}$) on the Transformer's original representational capacity ($p_{\text{sup}1} - p_{\text{sup}2}$); the results in Table~\ref{tab:tradeoff} show that this drop is negligible, with an average of only 0.0016 AUC points (approx. 0.2\%), demonstrating that the model internalizes pedagogical theory without sacrificing its expressive power; 2) We define the \textit{Interpretability Cost} (\textit{Int. Cost}) as the total performance impact of constraining final predictions to the causal logic of the reference model ($p_{\text{sup}1} - p_{\text{ref}}$); the results in Table~\ref{tab:tradeoff} show that the average cost across datasets is 0.0471 AUC points (5.9\% relative reduction), ranging from a minimum of 2.0\% (AS2015) to a maximum of 14.0\% (AS2009), which is highly acceptable given the substantial gains in transparency.
