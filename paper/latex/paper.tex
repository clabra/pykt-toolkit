...
This section details the experimental setup employed to train and evaluate gTransformer relative to various baseline models. It includes a description of the validation methodology, training setup, datasets, and benchmark models selected for comparative evaluation. 
...
\added[id=rev]{(2) grounded supervised predictions ($p_{\text{sup}2}$), from the standard supervised output;}
\added[id=rev]{(3) interpretable predictions ($p_{\text{ref}}$), derived exclusively from the BKT logic using grounded parameters; and}
\added[id=rev]{(4) population-level BKT predictions ($p_{\text{bkt}}$).}


\added[id=rev]{We establish three key metrics to quantify the trade-off:}
\begin{align}
...
\added[id=rev]{To verify that gTransformer's internal states are pedagogically valid, we implement a triple validation methodology based on the following hypotheses:}
...
\begin{itemize}
    ...
    \end{equation}
    \added[id=rev]{where $R^2_{\text{target}}$ represents the probe performance on true BKT targets and $R^2_{\text{control}}$ is the performance on a control task with randomly shuffled targets.}
    ...
    \end{equation}
    \added[id=rev]{where $w_1, w_2, w_3$ are relative weights. These dimensions include numerical proximity ($C_{\text{calibrated}}$), binary decision agreement ($C_{\text{directional}}$), and a dataset-wide rank of prediction disagreement ($C_{\text{percentile}}$).}
\end{itemize}
...
Students are \replaced[id=rev]{categorised}{categorized} into four learning situations based on their historical learning parameters. \Cref{fig:student_clusters_as2009} illustrates the distribution of students across the four quadrants:

\begin{itemize}
    \item \textbf{Low Initial Mastery / Low Learning Rate}: Observations for students in this quadrant suggest limited foundational mastery with slow skill acquisition. 
    \item \textbf{Low Initial Mastery / High Learning Rate}: Despite initial knowledge gaps, rapid progress indicates responsiveness to instruction. 
    \item \textbf{High Initial Mastery / Low Learning Rate}: Strong foundational mastery but minimal progress may signal disengagement or instructional mismatch. 
    \item \textbf{High Initial Mastery / High Learning Rate}: Consistent mastery and rapid skill development suggest readiness for advanced material.
\end{itemize}
...
In order to demonstrate the gTransformer's ability to capture historical learning context and overcome the Markovian limitations characteristic of traditional approaches, we show a comparative analysis of mastery trajectories for students with identical response sequences. 
