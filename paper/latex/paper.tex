...
\begin{figure}[!htbp]
    \centering
    \includegraphics[height=0.6\textheight, width=\textwidth, keepaspectratio]{img/idkt_arch_d2.png}
    \caption{The diagram illustrates the layers of the iDKT Architecture: (1) \text{Input Layer} enriched with BKT reference data ($p_{BKT}$ and parameters); (2) \text{Embeddings} layer providing the foundational material through individualized transformations ($x', y'$); (3) \text{Attention Heads} (Encoder-Decoder Transformer) synthesizing interaction history and current task into a contextualized representation ($z_t$); (4) \text{Prediction and Interpretability Heads} providing both performance probabilities ($p_{pred}$) and pedagogical diagnostics ($p_{know}, p_{rate}$); (5) \text{Loss Functions} implementing a multi-objective pipeline ($L_{pred}, L_{int}$) to ensure representational grounding; and (6) \text{Diagnostic Probing Validation} to verify the structural preservation of the pedagogical constructs through linear probes.}
    \label{idkt_arch}
\end{figure}
