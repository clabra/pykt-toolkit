...
\begin{figure}[!htbp]
    ...
    \caption{Theory-Guided Information Flow in gTransformer.}
    \label{fig_info_flow}
\end{figure}
...
\toprule
\textbf{Model} & \textbf{AS2009} & \textbf{AS2015} & \textbf{AL2005} & \textbf{Bridge2006} & \textbf{NIPS34} \\
\midrule
...
When comparing these results with baseline architectures, we observe that gTransformer achieves highly competitive performance across all benchmarks. On AS2009 and AS2015, gTransformer matches the top-performing AKT model exactly (0.7825 and 0.7081 respectively), demonstrating that pedagogical grounding does not compromise predictive accuracy. Notably, on AL2005, gTransformer achieves the highest performance (0.8306), matching the AKT reference value and substantially outperforming all other baselines including ATKT (0.7995) and DKT (0.8149). On Bridge2006, gTransformer achieves 0.8066, closely approaching the AKT reference value of 0.8208 while outperforming ATKT (0.7889). On NIPS34, gTransformer achieves 0.7908, surpassing all baselines except AKT (0.8033). These results provide empirical evidence that the model's pedagogical grounding does not compromise its predictive integrity, maintaining state-of-the-art performance while providing additional layers of interpretability. Furthermore, to test the hypothesis that interpretability constraints might degrade accuracy, we evaluated an \textit{unconstrained} variant of gTransformer (with all grounding loss terms $\lambda_{\text{ref}}$ and $\lambda_{\text{probe}}$ set to zero). On the primary AS2009 dataset, this unconstrained baseline achieved a Mean AUC of 0.7710, lower than the grounded model's 0.7825. This result suggests that the pedagogical regularization acts as a beneficial inductive bias, guiding the optimization toward more generalizable minima rather than hindering performance.

\subsection{Interpretability Validation (RQ1)} \label{sec_validation}

To verify that gTransformer’s internal representations—specifically the grounded BKT parameters ($p_{L_0}, p_T$) as defined in \cref{sec_grounded_params}—faithfully represent the educational constructs postulated by the reference model, we evaluate the representational alignment through two primary metrics that quantify the preservation and functional utility of pedagogical constructs. 
...
\begin{enumerate}
    ...
    \mathcal{I}_{1l} = \text{Corr}(p_{L_0,t}, P(L_{0})) \quad \text{and} \quad \mathcal{I}_{1t} = \text{Corr}(p_{T,t}, P(T))
    \end{equation}
    where $p_{L_0,t}$ and $p_{T,t}$ are the grounded BKT parameters as defined in \cref{sec_grounded_params}.
    \item \textbf{Predictor Equivalence (Functional Substitutability)}: This metric, denoted as $\mathcal{I}_2$, assesses whether the learned gTransformer parameters can functionally substitute the reference parameters in a simulation. We ``inject'' the projected factors ($p_{L_0,t}$ and $p_{T,t}$) into the canonical BKT recurrence equations to generate \textit{induced mastery trajectories}. $\mathcal{I}_2$ is defined as the correlation between these induced trajectories ($P(L)_{ind}$) and the reference trajectories ($P(L)_{ref}$) generated by the original BKT model:
    \begin{equation} \label{eq_i2}
    \mathcal{I}_2 = \text{Corr}\left( P(L)_{ind}, P(L)_{ref} \right)
    \end{equation}
\end{enumerate}

\Cref{tab_interpretability} summarizes the alignment metrics for the unconstrained gTransformer model ($\lambda_{\mathcal{I}}=0$), demonstrating that without explicit grounding, the deep model's latent space remains decoupled from pedagogical constructs.
...
Enabling Representational Grounding ($\lambda_{\mathcal{I}}=0.10$) leads to a significant restoration of semantic alignment, as evidenced in \cref{tab_interpretability_grounded}.
...
The results presented in \cref{tab_interpretability,tab_interpretability_grounded} reveal a notable divergence between \textit{Convergent Validity} ($\mathcal{I}_1$) and \textit{Predictor Equivalence} ($\mathcal{I}_2$). Although $\mathcal{I}_1$ attains high levels across most datasets even with relatively low $\lambda_{\mathcal{I}}$ grounding weights, $\mathcal{I}_2$ remains consistently low.
...
The probing protocol investigates whether the theoretical constructs incorporated in the grounding mechanism (\cref{sec_grounded_params}) remain dominant within the final contextualized representations ($z_t$) used for performance prediction. If the information remains linearly recoverable from $z_t$, it proves that the Transformer built its predictive logic upon the grounded foundations.
...
\Cref{tab_probing} presents the probing results for the grounded gTransformer model ($\lambda_{\mathcal{I}}=0.1$) on two representative datasets.
