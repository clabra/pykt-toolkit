...
\begin{itemize}
    \item \textbf{Low-to-Moderate Cost of Interpretability.} The average cost of interpretability across datasets is 0.0959 AUC points (12.1\% relative reduction), demonstrating that theory-grounded interpretability can be achieved with low performance degradation. The cost is highly consistent across all datasets, ranging from a minimum of 7.5\% (AS2015) to a maximum of 14.5\% (NIPS). 
    \item \textbf{General Interpretability Gain.} Comparing $p_{\text{ref}}$ against $p_{\text{bkt}}$ provides a measure of the \textit{Interpretability Gain}. In all four comparable datasets, interpretable neural predictions consistently outperform classical BKT: AS2009 (+10.4\%), Algebra 2005 (+2.0\%), Bridge to Algebra (+4.0\%), and NIPS (+19.4\%). This consistent improvement demonstrates that gTransformer provides a general diagnostic advantage over population-level priors. 
\end{itemize}
...
Our results validate the approach across three dimensions. First, we confirmed that gTransformer internalizes BKT constructs as its primary organizing principle, achieving high structural selectivity and semantic alignment with theoretical priors ($RQ1$). Second, quantifying the accuracy--interpretability trade-off, we showed that interpretable gTransformer predictions consistently surpass classical BKT with a low performance cost relative to black-box architectures ($RQ2$). Finally, we demonstrated that gTransformer enables context-aware personalization by capturing longitudinal learning patterns that Markovian models inherently overlook, allowing for differentiated diagnostics for students with identical scores but different interaction histories ($RQ3$).
