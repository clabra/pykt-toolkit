...
To verify that iDKT’s internal representations ($l_c, t_c, u_q$) reflect educational constructs throughout training, we establish a formal framework across five psychometric hypotheses:
...
\begin{enumerate}
    ...
    \item \textbf{$H_3$: Discriminant Validity (Construct Distinctness)}: Verifies that Knowledge Gap ($k_s$) and Learning Rate ($v_s$) are not perfectly collinear, proving the model can distinguish between prior knowledge and acquisition pace.
