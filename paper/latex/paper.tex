...
To address the challenge of achieving practical interpretability without sacrificing general applicability or predictive performance, we propose a new \textit{Theory-Grounded Interpretability} paradigm. This paradigm seeks to bridge the high capacity of deep learning with the semantic clarity of traditional models. To achieve this, we posit that an interpretable model must satisfy the following requirements:
\begin{enumerate}
    \item \textbf{State Estimations}: The model must produce explicit estimations of parameters corresponding to concepts used for semantic grounding.
    \item \textbf{Estimations Grounding}: These estimations must be aligned with the estimations of a intrinsically interpretable model.
    \item \textbf{Causal Derivation}: The predictions of the model are mathematically derived from the grounded estimations. 
\end{enumerate}

In this paper, we introduce \textit{Representational Grounding}, a methodology based on \textit{Theory-Grounded Embeddings}, that complies with these requierements and defines a rigorous metric to measure semantic alignment. 
