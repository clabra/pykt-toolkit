...
When comparing these results with baseline architectures, we observe that gTransformer achieves highly competitive performance across all benchmarks. On AS2009 and AS2015, gTransformer matches or exceeds the top-performing AKT model (0.7838 and 0.7081 respectively), demonstrating that pedagogical grounding does not compromise predictive accuracy. Notably, on AL2005, gTransformer achieves the highest performance (0.8306), matching the AKT reference value and substantially outperforming all other baselines including ATKT (0.7995) and DKT (0.8149). On BDG2006, gTransformer achieves 0.8066, closely approaching the AKT reference value of 0.8208 while outperforming ATKT (0.7889). On NIPS34, gTransformer achieves 0.7908, surpassing all baselines except AKT (0.8033). Furthermore, to test whether interpretability constraints degrade accuracy, we evaluated an \textit{unconstrained} gTransformer baseline (with $\lambda_{\text{ref}} = \lambda_{\text{probe}} = 0$). On AS2009, this baseline achieved 0.7710 AUC, lower than the grounded model's 0.7838, suggesting that theoretical grounding serves as a beneficial inductive bias that improves generalization.
...
\caption{Structural Fidelity for Initial Mastery ($L_0$). Parity plot showing strong linear recoverability ($r=0.698$, $R^2=0.487$) between BKT theoretical priors and probe-extracted parameters. Each bubble represents a binned aggregate of test interactions, with size encoding sample density. The tight adherence to the diagonal (red dashed) validates that latent representations preserve $L_0$ structure across the full parameter range. \label{fig_structural_fidelity_l0}}
\vspace{12pt}
\includegraphics[width=0.70\textwidth]{img/h1_structural_fidelity_t_fold_0_481134.png}
\caption{Structural Fidelity for Learning Rate ($T$). Parity plot demonstrating robust linear recoverability ($r=0.753$, $R^2=0.566$) with superior fidelity compared to $L_0$. The consistently tight clustering around the diagonal across all $T$ values indicates the model encodes learning rate dynamics with high precision. \label{fig_structural_fidelity_t}}
\end{figure}

\subsubsection{Semantic Grounding and Alignment Preservation (H1.2)} \label{sec_semantic_grounding}

For the second hypothesis H1.2 (Semantic Alignment), we evaluate whether grounded parameters preserve pedagogical semantics despite passing through multiple neural processing layers. A key risk in theory-guided deep learning is that models may use theoretical priors merely as initialization, subsequently ``repurposing'' them for black-box optimization that abandons educational meaning. \textbf{In contrast to H1.1, where we validated linear decodability of BKT constructs from latent representations using $R^2$, H1.2 validates monotonic relationship preservation through neural transformations.} We do not expect grounded parameters to exhibit linear relationships with theoretical priors because the model performs context-aware enrichment and student-specific individualization—transforming population-level priors into personalized mastery estimates. The critical validation question is whether this individualization preserves the pedagogical \textit{ordering} of mastery levels rather than mechanically reproducing exact numerical values.

We employ \textbf{Spearman's rank correlation ($\rho$)} as our validation metric, computed between individualized grounded parameters $\{p_{L_0,t}, p_{T,t}\}$ (after all transformer processing and contextual projection) and the original population-level BKT priors $\{\ell_{L0}, \ell_T\}$ used to initialize theoretical bases. Spearman is specifically designed to measure monotonic relationship preservation and \textbf{does not assume linearity}—it detects whether students with higher theoretical priors receive higher individualized parameters, regardless of the functional transformation applied. This makes it ideal for validating semantic alignment in models that intentionally individualize through non-linear neural processing. The metric is also immune to outliers, focusing on the pedagogical ordering that matters for educational assessment. Interpretation thresholds: $\rho \geq 0.6$ indicates strong alignment, $0.4 \leq \rho < 0.6$ indicates moderate alignment, and $\rho < 0.4$ indicates weak alignment.

Evaluating $N=52,825$ test interactions from AS2009, grounded parameters achieve Spearman rank correlations of $\rho_{L_0} = 0.427$ (moderate) for Initial Mastery and $\rho_T = 0.604$ (moderate-to-strong) for Learning Rate (\Cref{fig_semantic_alignment_l0,fig_semantic_alignment_t}). The Learning Rate parameter shows particularly strong preservation, approaching the 0.6 threshold for ``strong'' alignment, indicating the model reliably maintains theoretical understanding of how students improve with practice. As expected, grounded parameters show lower correlations than probe parameters from H1.1 ($\rho = 0.427$ vs 0.736 for $L_0$; $0.604$ vs 0.760 for $T$), confirming genuine student-specific individualization rather than trivial reproduction of population priors. The model successfully balances theoretical grounding with contextual refinement—neither abandoning priors (which would yield $\rho \approx 0$) nor mechanically echoing them (which would yield $\rho \approx 1$). Parity plots reveal that high-density regions (large bubbles) cluster near the theoretical ideal diagonal, demonstrating that typical predictions preserve pedagogical semantics while allowing adaptive refinement. \textbf{H1.2 is supported}: grounded parameters maintain moderate-to-strong monotonic alignment with theoretical priors while providing individualized mastery estimates.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.70\textwidth]{img/h12_recovery_l0_grounded_268444.png}
\caption{Semantic Alignment for Initial Mastery ($P_{L_0}$). Parity plot showing moderate monotonic relationship preservation (Spearman $\rho = 0.427$) between BKT theoretical priors and grounded parameters after neural transformation. Each bubble represents a binned aggregate of test interactions, with size encoding sample density. High-density regions (large bubbles) cluster near the diagonal (red dashed), demonstrating that rank-order preservation is robust where data is abundant while allowing individualization in sparse regions. \label{fig_semantic_alignment_l0}}
\vspace{12pt}
\includegraphics[width=0.70\textwidth]{img/h12_recovery_t_grounded_268444.png}
\caption{Semantic Alignment for Learning Rate ($P_T$). Parity plot demonstrating moderate-to-strong monotonic relationship preservation (Spearman $\rho = 0.604$) with superior alignment compared to $P_{L_0}$. The consistently tight clustering around the diagonal across the full $P_T$ range indicates the model reliably preserves pedagogical understanding of practice effects through all neural processing layers while enabling context-aware refinement. \label{fig_semantic_alignment_t}}
\end{figure}
