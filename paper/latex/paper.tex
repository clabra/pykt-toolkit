...
\begin{itemize}
    ...
    \item \textbf{Guess}: The probability of a correct response despite a lack of mastery; and 
...
Figure~\ref{fig_info_flow} illustrates the information flow through a gTransformer, mapped to the taxonomic dimensions of IML as defined by \citet{vonrueden2021informed}.


\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.58\textwidth]{img/iml.png}
...
\subsubsection{Difficulty-Aware Embeddings} \label{sec_embeddings}
...
\textbf{Interpretable Output Head}: The grounded BKT parameters $(p_{L_0,t}, p_{T,t})$ are passed through a differentiable implementation of Bayesian Knowledge Tracing logic, which performs a retrospective belief update walk over the student's interaction history. At each timestep, the BKT logic computes the probability of correctness $\hat{y}_{\text{ref},t}$ using the standard BKT update equations with the model's estimated mastery probabilities and the fixed population-level guess and slip parameters. This head ensures that the grounded parameters retain their intended pedagogical semantics by requiring them to produce valid predictions when subjected to BKT's causal reasoning process.
...
\subsubsection{Multi-Objective Loss}
...
\midrule
ASSISTments 2009 & 0.7783 $\pm$ 0.0009 & 0.6732 $\pm$ 0.0002 & 0.6097 $\pm$ 0.0008 & 0.1051 (13.5\%) \\
Algebra 2005 & 0.8219 $\pm$ 0.0011 & 0.5989 $\pm$ 0.0003 & 0.7215 $\pm$ 0.0014 & 0.2230 (27.1\%) \\
ASSISTments 2015 & 0.7073 $\pm$ 0.0007 & 0.6233 $\pm$ 0.0009 & --- & 0.0840 (11.9\%) \\
Bridge to Algebra & 0.8120 $\pm$ 0.0009 & 0.5559 $\pm$ 0.0004 & 0.6756 $\pm$ 0.0017 & 0.2561 (31.5\%) \\
NIPS Task 3/4 & 0.7990 $\pm$ 0.0005 & 0.7020 $\pm$ 0.0004 & 0.5729 $\pm$ 0.0004 & 0.0970 (12.1\%) \\
\midrule
\textbf{Mean} & \textbf{0.7837} & \textbf{0.6307} & \textbf{0.6449} & \textbf{0.1530 (19.5\%)} \\
\bottomrule
\end{tabular}
\end{table}

The results reveal three critical insights regarding the accuracy--interpretability trade-off:

\textbf{(1) Moderate Cost of Interpretability.} The average cost of interpretability across datasets is 0.153 AUC points (19.5\% relative reduction), demonstrating that theory-grounded interpretability can be achieved with moderate performance degradation. The cost varies substantially across datasets, ranging from 11.9\% (ASSISTments 2015) to 31.5\% (Bridge to Algebra), suggesting that the penalty depends on the complexity of skill structures and the degree to which BKT's simplifying assumptions align with the underlying learning dynamics.

\textbf{(2) Dataset-Dependent Trade-Offs.} Two distinct patterns emerge: (a) \textit{Low-cost datasets} (ASSISTments 2009, ASSISTments 2015, NIPS Task 3/4) exhibit costs below 14\%, indicating that BKT's theoretical framework provides a reasonable approximation of the learning process; (b) \textit{High-cost datasets} (Algebra 2005, Bridge to Algebra) show costs exceeding 27\%, suggesting more complex skill dependencies that cannot be captured by BKT's Markovian assumptions. Notably, in these high-cost datasets, the classical BKT baseline ($p_{\text{bkt}}$) actually outperforms the interpretable neural predictions ($p_{\text{ref}}$), indicating that the grounding constraints may be too restrictive for certain problem structures.

\textbf{(3) Gain from Neural Individualization.} Comparing $p_{\text{ref}}$ against $p_{\text{bkt}}$ reveals the value of student-specific parameter individualization. In three of four comparable datasets (ASSISTments 2009, NIPS Task 3/4), interpretable predictions significantly outperform classical BKT (+6.4\% and +12.9\% AUC respectively), demonstrating that neural contextualization provides meaningful diagnostic improvements over population-level priors. However, the negative gains in Algebra 2005 and Bridge to Algebra suggest that question-level late fusion may not be the optimal evaluation protocol for all datasets, potentially favoring skill-level assessments or alternative aggregation strategies.

In summary, the cost of interpretability is generally moderate and acceptable for practical educational applications, particularly in datasets where BKT's theoretical assumptions hold. The heterogeneity across datasets underscores the importance of matching interpretability frameworks to domain characteristics rather than applying a one-size-fits-all approach.
...
\begin{itemize}
    \item \textbf{Non-Markovian Initialization}: At the very first interaction ($t=0$), gTransformer already provides divergent estimations. Student 2208 is recognized as having a significantly higher foundational baseline for this concept than Student 167, despite both having the same immediate local history.
    \item \textbf{Velocity-Driven Cross-Over}: Despite starting from a lower baseline, Student 167 exhibits a steeper learning curve (higher $v_{s,t}$). After the first success, their mastery estimation increases rapidly, eventually overtaking Student 2208 at the ``Cross-Over Point'' (approximately $t=1.5$).
\end{itemize}
...
As illustrated in \cref{fig_clusters}, gTransformer maps the student population into a continuous \textit{diagnostic manifold} defined by two student-specific parameters: the Knowledge Gap ($k_{s,t}$, representing placement) and the Learning Velocity ($v_{s,t}$, representing pacing).
 Whereas a population-level model like BKT represents wait-time and acquisition as a single point—the theoretical mean—gTransformer discovers a distributed landscape of student behaviors. Through unsupervised clustering, we identify four distinct learning archetypes: \textbf{Initial Standing} (Low $k_s$, Low $v_s$), \textbf{Developing Pacing} (Low $k_s$, Moderate $v_s$), \textbf{High-Velocity Growth} (Low $k_s$, High $v_s$), and \textbf{Advanced Standing} (High $k_s$, High $v_s$). 
...
\caption{Discovery of student archetypes in the gTransformer diagnostic space. Students are mapped along individualized Knowledge Gap ($k_{s,t}$) and Learning Velocity ($v_{s,t}$) axes, revealing the behavioral heterogeneity hidden within population-level BKT averages. \label{fig_clusters}}
\end{figure}

\subsubsection{High-Granularity Trajectory Analysis} \label{sec_mosaic_analysis}

To provide visual evidence for RQ3, we generated the mosaic showed in Figure~\ref{fig_mosaic} displaying individualized trajectories for three representative individual students (Fast, Median, Slow) per skill across the 15 most frequent skills in the dataset (including major topics like Circle Graphs, Venn Diagrams, and Linear Equations).
