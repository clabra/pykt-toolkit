...
\subsection{Tradeoff between Predictive Performance and Interpretability (RQ1)} \label{sec_tradeoff}
...
\vspace{2mm}
\raggedright
\caption{Comparison of predictive performance across various benchmark datasets. The table presents the test Area Under the Curve (AUC) for gTransformer and baseline models. Performances are evaluated at the question level using the mean late-fusion protocol to ensure fair comparison across diverse architectures. Values in bold indicate the best-performing model for each dataset. \label{tab_performance}}
\end{table}

Compared to baseline models, gTransformer achieves highly competitive performance across all benchmarks. For this comparison, we evaluate a configuration with interpretability mechanisms ablated ($\lambda_{\text{ref}} = 0, \lambda_{\text{probe}} = 0$) to establish its maximum predictive capacity in the absence of interpretability constraints. Hyperparameter settings for each dataset are detailed in \cref{tab:implementation_params}. On AS2009, this configuration achieves an AUC of 0.7831, outperforming all other evaluated models. It ranks second on AL2005, BDG2006 and NIPS34 (surpassed only by AKT), and third on AS2015 (behind ATKT and AKT). These results demonstrate gTransformer's flexibility in delivering top-tier predictive performance while retaining its architectural support for theory-based interpretability, offering a robust and transparent alternative for high-stakes educational environments where pedagogical grounding is paramount.


The grounded (non-ablated) gTransformer model generates two distinct outputs: (1) supervised predictions from the standard output head ($p_{\text{sup}2}$), and (2) interpretable BKT logic predictions ($p_{\text{ref}}$) derived from the student-specific grounded parameters ($p_{L_0}$, $p_T$). To evaluate the performance trade-off, we compare these against (3) the ablated configuration ($p_{\text{sup}1}$), which establishes the maximum predictive capacity in the absence of interpretability constraints, and (4) the classical BKT baseline ($p_{\text{bkt}}$) using population-level parameters. This baseline is omitted for AS2015 because the test data lacks the question-level identifiers required for evaluation. Table \cref{tab:tradeoff} presents the test AUC results for these outputs, quantifying the Interpretability Cost ($p_{\text{sup}1} - p_{\text{ref}}$) and the Interpretability Gain ($p_{\text{ref}} - p_{\text{bkt}}$). The hyperparameter configuration for the grounded model is detailed in Table \cref{tab:params}.
...
\midrule
AS2009 & 0.7831 & 0.7814 & 0.7436 & 0.6097 & 0.0017 & 0.0378 (4.8\%) & 0.1339 (22.0\%) \\
AS2015 & 0.7078 & 0.7070 & 0.6940 & --- & 0.0008 & 0.0130 (1.8\%) & --- \\
AL2005 & 0.8240 & 0.8237 & 0.7800 & 0.7215 & 0.0003 & 0.0437 (5.3\%) & 0.0585 (8.1\%) \\
Bridge2006 & 0.8148 & 0.8107 & 0.7810 & 0.6756 & 0.0041 & 0.0297 (3.7\%) & 0.1054 (15.6\%) \\
NIPS34 & 0.8006 & 0.7987 & 0.7666 & 0.5729 & 0.0019 & 0.0321 (4.0\%) & 0.1937 (33.8\%) \\
\midrule
\textbf{Mean} & 0.7861 & 0.7845 & 0.7530 & 0.6449 & 0.0018 & 0.0313 (3.9\%) & 0.1229 (19.9\%) \\
\bottomrule
\end{tabular}
...
ablation & \texttt{all} & \texttt{none} \\
$\lambda_{\text{sup}}$ & 1.0 & 1.0 \\
$\lambda_{\text{ref}}$ & \textbf{0} & 0.5 \\
$\lambda_{\text{probe}}$ & \textbf{0} & 1.0 \\
\midrule
\multicolumn{3}{l}{\textit{Architecture Configuration}} \\
$d_{\text{model}}$ & 64 & 64 \\
$n_{\text{blocks}}$ & 4 & 4 \\
$d_{\text{ff}}$ & 256 & 256 \\
num. attn. heads & 4 & \makecell{AS2009: 8\\others: 4} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Hyperparameter configurations for \cref{tab_performance} (Predictive Performance with ablation=all) and \cref{tab:tradeoff} (Accuracy-Interpretability Trade-off with ablation=none). \Cref{tab_performance} disables grounding and probing for maximum predictive performance, while \cref{tab:tradeoff} uses full interpretability with $\lambda_{\text{ref}}=0.5$ and $\lambda_{\text{probe}}=1.0$.}
\label{tab:params}
\end{table}
...
\begin{itemize}
\item \textbf{H2.1: Structural Encoding}
...
The objective of this triple validation is to demonstrate that grounding constraints actively shape the model's internal structure, rather than merely correlating with outputs, to produce interpretable predictions. It addresses a fundamental challenge in educational AI: whether deep learning models can internalize pedagogical theory, rather than learning arbitrary features that happen to yield accurate predictions. We evaluate these hypotheses using the AS2009 dataset, one of the most widely-used benchmarks in knowledge tracing research, offering rich student interaction data and demonstrating strong predictive performance with gTransformer (AUC = 0.7814), making it an ideal testbed for examining the interplay between interpretability and model capacity.

\subsubsection{Structural Encoding via Diagnostic Probing (H2.1)} \label{sec_structural_alignment}
...
\Cref{fig_structural_fidelity_l0,fig_structural_fidelity_t} present the diagnostic probing results on AS2009 via parity plots, comparing BKT theoretical priors (x-axis) with probe-extracted parameters (y-axis). Each point represents an aggregate of interactions, with its size proportional to the sample density. The tight adherence to the diagonal confirms a high degree of linear recoverability, with the obtained values indicating strong structural encoding:

\begin{itemize}
    \item \textbf{Initial Mastery ($L_0$)}: Achieves fidelity $R^2 = 0.549 \pm 0.064$ with Pearson $r = 0.743 \pm 0.041$, indicating strong linear recoverability. The selectivity $\Delta R^2 = 0.619 \pm 0.073$ exceeds the threshold of 0.5, demonstrating that $L_0$ is a dominant organizing principle in the latent space rather than an incidental pattern. The control task yields a negative $R^2 = -0.069$ (indicating performance worse than a mean-based baseline), confirming that probes cannot recover shuffled targets.
    
    \item \textbf{Learning Rate ($T$)}: Shows strong structural encoding with fidelity $R^2 = 0.515 \pm 0.080$ and Pearson $r = 0.721 \pm 0.050$. The selectivity reaches $\Delta R^2 = 0.570 \pm 0.070$, demonstrating robust encoding of learning rate dynamics, with control $R^2 = -0.055$.
\end{itemize}

\textbf{Interpretation}: The high selectivity ($>0.5$) for both parameters validates H2.1 (Structural Encoding) hypothesis: BKT constructs are preserved through all Transformer layers and remain as primary organizing axes in final representations used for prediction. 
...
\caption{Structural Fidelity for Initial Mastery $P_{L_0}$. Plot showing strong linear recoverability ($R^2=0.549$, $r=0.743$, $\Delta=0.619$) between BKT theoretical priors and probe-extracted parameters. Each bubble represents a binned aggregate of test interactions, with size encoding sample density. The tight adherence to the diagonal and high selectivity ($\Delta > 0.5$) validate that $P_{L_0}$ is a dominant organizing principle in latent representations. \label{fig_structural_fidelity_l0}}
\vspace{12pt}
\includegraphics[width=0.70\textwidth]{img/h12_recovery_t_probe_893468.png}
\caption{Structural Fidelity for Learning Rate $P_T$. Plot demonstrating robust linear recoverability ($R^2=0.515$, $r=0.721$, $\Delta=0.570$) between BKT theoretical priors and probe-extracted parameters. The tight clustering around the diagonal and high selectivity ($\Delta > 0.5$) confirm that learning rate dynamics are dominantly encoded in latent representations. \label{fig_structural_fidelity_t}}
\end{figure}

\subsubsection{Semantic Alignment (H2.2)} \label{sec_semantic_alignment}

Hypothesis H2.2 examines whether grounded parameters $\{p_{L_0,t}, p_{T,t}\}$ retain their pedagogical semantics after processing, ensuring models do not repurpose theoretical constructs for black-box optimization. Unlike H2.1, which probes latent representations, H2.2 validates monotonic relationship preservation in final projected parameters. We use Spearman's rank correlation ($\rho$) to measure alignment, as it captures monotonic consistency without assuming linearity ($\rho \geq 0.6$ strong, $0.4 \leq \rho < 0.6$ moderate).

\Cref{fig_semantic_alignment_l0,fig_semantic_alignment_t} present parity plots comparing BKT theoretical priors (x-axis) with grounded parameters (y-axis), where each bubble represents an aggregate of test interactions with size encoding sample density. The patterns reveal differential preservation strategies:

\begin{itemize}
    \item \textbf{Initial Mastery ($P_{L_0}$)} shows weak alignment ($\rho = 0.360$, $r = 0.353$) with substantial scatter, indicating strong student-specific individualization. However, MAE = $0.168$ validates that refinements remain within pedagogical bounds, the model neither abandons nor mechanically reproduces theoretical priors.
    \item \textbf{Learning Rate ($P_T$)} demonstrates moderate alignment ($\rho = 0.544$, $r = 0.470$) with clustering around the diagonal. The low MAE = $0.092$ confirms the model preserves pedagogical understanding of practice effects while enabling context-aware refinement.
\end{itemize}

These findings support H2.2: grounded parameters maintain meaningful monotonic alignment with theory while capturing individual heterogeneity. The differential alignment ($P_T$ moderate, $P_{L_0}$ weak) reveals that learning rates preserve more theoretical structure while initial mastery estimates undergo stronger individualization.


\begin{figure}[htbp]
\centering
\includegraphics[width=0.70\textwidth]{img/h12_recovery_l0_grounded_893468.png}
...
\vspace{12pt}
\includegraphics[width=0.70\textwidth]{img/h12_recovery_t_grounded_893468.png}
...
\subsubsection{Functional Alignment and Prediction Confidence (H2.3)} \label{sec_functional_alignment}
