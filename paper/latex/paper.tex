...
\begin{equation}
    \mathcal{L}_{total} = \mathcal{L}_{sup} + \lambda_{ref} \mathcal{L}_{ref} + \lambda_{know} \mathcal{L}_{know} + \lambda_{rate} \mathcal{L}_{rate} + \lambda_{reg} \mathcal{L}_{reg}
\end{equation}
...
\end{align}
where $p_{know} = \sigma(\bar{l}_c)$ and $p_{rate} = \sigma(\bar{t}_c)$ are the diagnostic read-outs. These are obtained by calculating the collective logit (the mean across the $d_{model}$ latent embedding dimensions) followed by a sigmoid projection to the $[0, 1]$ interval. This formulation forces the high-dimensional latent representations to be semantically consistent with the theoretical constructs of the reference model. To ensure signal persistence through the model's normalization layers, the base embeddings $\mathbf{l}_0$ and $\mathbf{T}$ are initialized using \textit{Textured Grounding}, where dimensions are populated with small-variance Gaussian noise shifted by the respective theoretical logit.
...
The results presented in Tables \ref{tab_interpretability} and \ref{tab_interpretability_grounded} reveal a notable divergence between \textit{Convergent Validity} ($\mathcal{I}_1$) and \textit{Predictor Equivalence} ($\mathcal{I}_2$). Although $\mathcal{I}_1$ attains high levels across most datasets even with relatively low $\lambda_{ref}$ grounding weights, $\mathcal{I}_2$ remains consistently low.
...
While the convergent validity metrics ($\mathcal{I}_1, \mathcal{I}_2$) provide evidence that the model's inputs are successfully grounded, a critical scientific question remains: does this pedagogical signal survive the Transformer's deep processing? High-capacity models can often satisfy input-level constraints while discarding that information in favor of non-interpretable patterns within their deeper layers. To verify the terminal link in our \textit{Chain of Fidelity}, we employ \textit{Diagnostic Probing with Control Tasks} \citep{hewitt2019designing} to validate the \textit{Structural Preservation} of constructs at the point of prediction.

The probing protocol investigates whether the theoretical constructs injected into the embedding layer (Equations \ref{eq_lc} and \ref{eq_tc}) remain dominant within the final contextualized representations ($z_t$) used for performance prediction. If the information remains linearly recoverable from $z_t$, it proves that the Transformer core has not merely simulated the grounding target but has actively built its predictive logic upon those grounded foundations.
...
where $R^2_{\text{true}}$ measures the variance explained when predicting actual BKT mastery, and $R^2_{\text{control}}$ measures the same for the shuffled control. A high positive selectivity score ($\Delta R^2 > 0.5$) provides terminal evidence of structural preservation, confirming that the pedagogical constructs remain the primary informative features through the model's entire reasoning chain.
