...
Our approach can be formally positioned within the taxonomies of Informed Machine Learning proposed by \citet{vonrueden2021informed} and adapted for Knowledge Tracing by \citet{zanellati2024hybrid}. Specifically, iDKT implements distinct integration strategies across all knowledge dimensions. As a \textit{Knowledge Source}, we utilize "Learning Knowledge" derived from pedagogical assumptions and cognitive theories found in BKT. For \textit{Knowledge Representation}, we employ "Simulation Results," where an interpretable reference model functions as a simulator to generate prior probability distributions. Finally, for \textit{Knowledge Integration}, we adopt a multi-faceted strategy: (a) utilizing a "Training Dataset" augmented with BKT outputs, (b) constraining the "Hypothesis Set" through inference biases that favor solutions aligned with the reference theory—inspired by the "Rashomon Set" argument for interpretability \citep{rudin2019stop}—and (c) modifying the "Learning Algorithm" via a physics-guided loss function \citep{willard2022integrating}, which penalizes divergent latent states. This framework corresponds to what \citet{battaglia2018relational} define as a Relational Inductive Bias, enforcing a structural isomorphism between the model’s internal representations and the theoretical constructs they are intended to capture.
...
% Nodes
\node[title] (t1) {Knowledge Source};
\node[box, below=0.2cm of t1, fill=blue!10] (source) {Learning Knowledge\\(Pedagogical Theory)};

\node[title, right=of t1] (t2) {Representation};
\node[box, below=0.2cm of t2, fill=green!10] (rep) {Simulation Results\\(BKT Reference)};

\node[title, right=of t2] (t3) {Integration};
\node[box, below=0.2cm of t3, fill=orange!10] (data) {Training Data\\(Augmentation)};
\node[box, below=0.3cm of data, fill=orange!10] (hypo) {Hypothesis Set\\(Inductive Bias)};
\node[box, below=0.3cm of hypo, fill=orange!10] (algo) {Learning Algorithm\\(Physics-Guided Loss)};

% Edges
\draw[arrow] (source) -- (rep);
\draw[arrow] (rep) -- (data);
\draw[arrow] (rep.east) -- ++(0.5,0) |- (hypo.west);
\draw[arrow] (rep.east) -- ++(0.5,0) |- (algo.west);

\end{tikzpicture}
\caption{Taxonomy of Informed Machine Learning applied to iDKT. The framework integrates pedagogical theory into the deep learning pipeline through three distinct channels: data augmentation, structural inductive bias, and loss function constraints.}
\label{fig:iml_taxonomy}
\end{figure}
...
\subsection{Reference Model: Individualized Bayesian Knowledge Tracing}

Bayesian Knowledge Tracing (BKT) \citep{corbett1994knowledge} models student mastery as a latent variable $L_t \in \{0, 1\}$ that transitions based on learning opportunities. BKT is governed by four parameters that map directly to latent and observed behavior:
\begin{itemize}
    \item $P(L_0)$: Initial Mastery — the probability that the student already knows the skill before the first interaction.
    \item $T$: Transit probability — the probability of learning a skill after an interaction.
    \item $G$: Guess probability — the probability of a student getting the correct answer despite not having mastered the skill.
    \item $S$: Slip probability — the probability of a student making a mistake despite having mastered the skill.
\end{itemize}
