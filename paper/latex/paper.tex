...
Figure \ref{fig_pareto_as2009} illustrates the resulting Pareto frontier for ASSISTments 2009. We observe a distinctive \textit{synergy point} at $\lambda=0.10$, where the inclusion of the pedagogical prior actually improves predictive performance (+0.0048 AUC points) compared to the unconstrained baseline. This indicates that for complex student behavior, the BKT-guided inductive bias serves as a highly effective regularizer. However, as the constraint weight exceeds $\lambda=0.3$, we observe a \textit{grounding collapse}, where the model's capacity to represent behavioral nuances is over-restricted by the theoretical prior, leading to a simultaneous drop in both accuracy and interpretability.
...
0.1 (Synergy Spot) & 0.8255 & 0.3846 & Enhanced Performance \\
0.2 & 0.8110 & 0.4270 & Theory-Informed \\
0.3 & 0.7963 & 0.4790 & Peak Alignment \\
0.5 & 0.7760 & 0.4125 & Saturated \\
1.0 & 0.7501 & 0.0714 & Theoretical Collapse \\
\bottomrule
\end{tabularx}
