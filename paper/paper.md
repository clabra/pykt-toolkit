...
While deep knowledge tracing (DKT) models provide high predictive accuracy, their "black-box" nature limits the extraction of actionable pedagogical insights. This study introduces iDKT, an interpretable-by-design Transformer model that utilizes structural grounding to anchor deep latent representations to educational constructs defined by pedagogical theories. Using Bayesian Knowledge Tracing (BKT) as a reference model, we evaluated iDKT across multiple real-world datasets and established a formal validation framework to verify its internal representations. Results demonstrate that iDKT maintains state-of-the-art performance while providing significantly higher granularity than population-level baselines. Specifically, the model identifies student-specific knowledge gaps and learning velocities, providing mastery estimations that are more sensitive to the nuances of individual behavioral patterns than standard theoretical predictions. By anchoring deep learning to pedagogical theory, iDKT enables precise diagnostic placement and dynamic pacing in adaptive learning environments. This work offers both a robust methodology for evaluating model interpretability and a practical tool for improving educational effectiveness through data-driven personalization.
