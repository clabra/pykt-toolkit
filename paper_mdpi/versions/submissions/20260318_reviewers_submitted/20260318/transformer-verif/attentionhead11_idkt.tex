...
\foreachvector{\draw[arrow] (w_\vidx) -- (z_\vidx);}
\addglobalfunction{w_1, anchor=west}{hmask}{\hmask}
...
\foreach \vidx in {1,...,\numnodes} {
    \draw[arrow] (z_\vidx) -- (key_\vidx) \ifnum \vidx=1 node[midway, left] {\scriptsize key} \fi;
    \draw[arrow] (z_\vidx) -- (value_\vidx) \ifnum \vidx=1 node[midway, right] {\scriptsize value} \fi;}
...
\foreachvector{\draw[arrow] (kprime_\vidx) -- (vprime_\vidx);}


\node[invisible, above={6*\vertispace + 4*\minheight} of z_0] (result_0) {};
\addlayer[value]{result}
\foreachvector{\draw[arrow] (vprime_\vidx) -- (result_\myposition);}
