"""
GainAKT3: Gain-based Attention Knowledge Tracing Model
...
class EncoderBlock(nn.Module):
    """
    A single Transformer Encoder block for the GainAKT3 model.
...
class GainAKT3(nn.Module):
    """
    GainAKT3 model for Knowledge Tracing using a gain-based attention mechanism.
    ...
    def __init__(self, num_c, seq_len=200, d_model=128, n_heads=8, num_encoder_blocks=2, 
        ...
        if self.use_skill_difficulty:
            ...
            print(f"[GainAKT3] Skill difficulty (embedding modulation) enabled: +{num_c} parameters")
        ...
        if self.use_student_speed:
            ...
            print(f"[GainAKT3] Student learning speed enabled: +{num_students * 16} parameters ({num_students} students)")
    ...
    def forward(self, q, r, qry=None, qtest=False, student_ids=None):
        """
        Forward pass for the GainAKT3 model, following PyKT conventions.
