    ...
    **Benefits**:
    ...
    def __init__(self, num_c, seq_len=200, d_model=128, n_heads=8, num_encoder_blocks=2, 
        ...
        **DO NOT RELY ON THESE DEFAULTS IN PRODUCTION CODE**
        ...
        # V3 (2025-11-18): Beta spread initialization - use normal distribution to create initial variance
        # Previous experiments (992161, 178361, etc.) showed uniform initialization → uniform convergence
        # Solution: Start with spread (mean=beta_skill_init, std=0.5) to provide initial differentiation signal
        beta_init_values = torch.randn(num_c) * 0.5 + beta_skill_init  # N(beta_skill_init, 0.5)
        beta_init_values = torch.clamp(beta_init_values, min=0.5, max=5.0)  # Constrain to reasonable range
        self.beta_skill = torch.nn.Parameter(beta_init_values)  # Skill difficulty (curve steepness)
    ...
    def forward_with_states(self, q, r, qry=None, batch_idx=None, student_ids=None, student_velocities=None):
        ...
        # V3 (2025-11-18): Add skill-contrastive loss to prevent uniform gains
        # Cross-skill comparison mechanism - encourages different skills to have different gain patterns
        # This addresses "lazy solution" problem observed in experiments 992161, 178361, 218502, etc.
        skill_contrastive_loss = self.compute_skill_contrastive_loss(skill_gains)  # Scalar loss
        
        # V3 (2025-11-18): Add beta spread regularization to prevent parameter collapse
        # Maintains beta_skill variance during training (target: std > 0.3)
        # Prevents beta parameters from collapsing to uniform values
        ...
        # Do NOT override base predictions - keep both for dual loss computation
            beta_spread_regularization = None  # V3 (2025-11-18): Initialize for output dict
        ...
        # V3 (2025-11-18): Add skill-contrastive loss to output
        if skill_contrastive_loss is not None:
            output['skill_contrastive_loss'] = skill_contrastive_loss  # Scalar loss preventing uniform gains
        
        # V3 (2025-11-18): Add beta spread regularization to output
        if beta_spread_regularization is not None:
            output['beta_spread_regularization'] = beta_spread_regularization  # Scalar loss maintaining beta variance
    ...
    def compute_skill_contrastive_loss(self, skill_gains):
        """
        Compute skill-contrastive loss to prevent uniform gains across skills.
        
        V3 ENHANCEMENT (2025-11-18): Cross-skill comparison mechanism
        
        Rationale:
        Previous experiments (992161, 178361, 218502, 999787, 995130, 820618) showed
        "lazy solution" problem - all skills converged to uniform gains (std<0.002)
        regardless of BCE weight (0%-100%). This indicates optimization trajectory problem.
        
        Solution: Explicit cross-skill comparison loss
        - Computes pairwise differences between skill gains at each timestep
        - Penalizes when skill gains are too similar (low variance)
        - Encourages skill differentiation through contrastive mechanism
        
        Formula:
            For each timestep t:
            - skill_diffs = pairwise_differences(skill_gains[:, t, :])
            - loss = -mean(abs(skill_diffs))  # Negative = maximize differences
        
        Args:
            skill_gains: [batch_size, seq_len, num_c] - per-skill gain predictions
            
        Returns:
            torch.Tensor: Scalar contrastive loss (negative = encourage high variance)
        """
        batch_size, seq_len, num_c = skill_gains.shape
        
        # Average gains across batch and time to get per-skill signature [num_c]
        # This creates a skill-level summary that should differentiate across skills
        avg_skill_gains = skill_gains.mean(dim=(0, 1))  # [num_c]
        
        # Compute pairwise L1 distances between skills
        # Expand dims: [num_c, 1] and [1, num_c] → broadcast → [num_c, num_c]
        skill_diffs = torch.abs(
            avg_skill_gains.unsqueeze(0) - avg_skill_gains.unsqueeze(1)
        )  # [num_c, num_c]
        
        # Average pairwise difference (exclude diagonal = self-comparison)
        mask = 1.0 - torch.eye(num_c, device=skill_gains.device)
        avg_diff = (skill_diffs * mask).sum() / (num_c * (num_c - 1))
        
        # Return negative loss to MAXIMIZE differences (contrastive objective)
        # When used with positive weight, this encourages skill differentiation
        return -avg_diff
    
    def compute_beta_spread_regularization(self):
        """
        Compute beta spread regularization to prevent beta_skill collapse.
        
        V3 ENHANCEMENT (2025-11-18): Maintain skill difficulty variation
        
        Rationale:
        Beta spread initialization provides initial variance, but without explicit
        regularization, beta_skill parameters can collapse to uniform values during training.
        This regularization maintains the spread to preserve skill differentiation.
        
        Target: std(beta_skill) > 0.3 (maintains meaningful difficulty differences)
        
        Formula:
            loss = max(0, 0.3 - std(beta_skill))^2
            
        When std < 0.3: Penalty increases quadratically
        When std >= 0.3: No penalty (already diverse)
        
        Returns:
            torch.Tensor: Scalar regularization loss (0 if std >= 0.3)
        """
        beta_std = self.beta_skill.std()
        target_std = 0.3
        
        # Hinge loss: penalize when std drops below target
        penalty = torch.clamp(target_std - beta_std, min=0.0)
