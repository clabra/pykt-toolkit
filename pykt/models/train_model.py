...
def cal_loss(model, ys, r, rshft, sm, cshft, preloss=[]):
    ...
    elif model_name in ["akt","gtransformer","extrakt","folibikt", "robustkt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx","lefokt_akt", "dtransformer", "simakt", "fluckt"]:
        y = torch.masked_select(ys[0], sm)
        t = torch.masked_select(rshft, sm)
...
def model_forward(model, data, rel=None):
    ...
    if model_name in ["dkt"]:
        ...
        if model_name == "gtransformer":
            ...
            if uids is not None:
                uids = uids.to(device)
            y, im, rate, reg_loss, reg_losses = model(cc.long(), cr.long(), cq.long(), uid_data=uids)
        else:
