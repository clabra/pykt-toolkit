#!/usr/bin/env python3
"""
CPU-Safe Test Evaluation with Fresh Data Processing
"""
...
def clear_cuda_pickle_files():
    """Remove CUDA-contaminated pickle files to force fresh CPU processing."""
    pickle_patterns = [
        "/workspaces/pykt-toolkit/data/assist2015/*_-1*.pkl",
        "/workspaces/pykt-toolkit/data/assist2015/test_*.pkl"
    ]
    ...
    for pattern in pickle_patterns:
        for file_path in glob.glob(pattern):
            try:
                ...
                print(f"🗑️  Removed CUDA pickle: {os.path.basename(file_path)}")
...
def evaluate_test_fresh(model_path: str):
    """Evaluate on test data with fresh CPU processing."""
    ...
    logger.info("🚀 STARTING FRESH CPU TEST EVALUATION")
    logger.info("=" * 60)
    logger.info("⚠️  CORRECTING DATA LEAKAGE: Using actual test data")
    logger.info("🗑️  Forcing fresh data processing on CPU")
    
    # Clear CUDA-contaminated pickle files
    removed_files = clear_cuda_pickle_files()
    logger.info(f"🧹 Cleared {len(removed_files)} CUDA pickle files")
    ...
    # Create test dataset directly (forcing fresh processing)
    test_file = "/workspaces/pykt-toolkit/data/assist2015/test_sequences.csv"
    logger.info(f"Creating fresh test dataset from: {test_file}")
    
    try:
        # Create fresh test dataset on CPU (this will process from scratch)
        test_dataset = KTDataset(test_file, ["concepts"], {-1})
        test_loader = DataLoader(test_dataset, batch_size=16, shuffle=False, num_workers=0)
        logger.info(f"✓ Fresh test dataset created: {len(test_dataset)} sequences, {len(test_loader)} batches")
    except Exception as e:
        logger.error(f"❌ Failed to create test dataset: {e}")
        return None
    
    # Evaluate
    logger.info("🧪 Evaluating on fresh test dataset...")
    ...
    # Compare with validation performance (CORRECTED - was using leaked data)
    validation_auc = 0.7210  # This was actually from validation fold 0 during training
    ...
    results = {
        ...
        "evaluation_type": "FRESH_TEST_DATASET_CPU_ONLY",
        "data_leakage_corrected": True,
        "fresh_processing": True,
        "performance_metrics": {
            "test_auc": float(test_auc),
            "test_accuracy": float(test_accuracy),
            "validation_auc_original": validation_auc,
        ...
        "dataset_info": {
            ...
            "device": "cpu",
            "cuda_files_cleared": len(removed_files)
        }
    }
    ...
    logger.info(f"📊 VALIDATION AUC was: {validation_auc:.4f} (on fold 0 - same as benchmark!)")
...
if __name__ == "__main__":
    ...
    evaluate_test_fresh(args.model_path)